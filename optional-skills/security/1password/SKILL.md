...
version: 1.0.0
author: arceus77-7, enhanced by Caesar Agent
license: MIT
metadata:
  caesar:
...
- `tmux` available for stable authenticated sessions during Caesar terminal calls (desktop app flow only)
...
### Service Account (recommended for Caesar)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.caesar/.env` (the skill will prompt for this on first load).
...
## Caesar Execution Pattern (desktop app flow)

Caesar terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/caesar-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/caesar-op.sock"
