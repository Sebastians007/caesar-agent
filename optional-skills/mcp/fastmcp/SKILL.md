...
- smoke-test a server with the FastMCP CLI before wiring it into Caesar or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Caesar. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.caesar/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.caesar/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Caesar integration, either:

- configure the server in `~/.caesar/config.yaml` using the `native-mcp` skill, or
...
### Caesar cannot see the deployed server

The server-building part may be correct while the Caesar config is not. Load the `native-mcp` skill and configure the server in `~/.caesar/config.yaml`, then restart Caesar.
