---
name: telephony
description: Give Caesar phone capabilities without core tool changes. Provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi.
version: 1.0.0
...
This optional skill gives Caesar practical phone capabilities while keeping telephony out of the core tool list.

It ships with a helper script, `scripts/telephony.py`, that can:
- save provider credentials into `~/.caesar/.env`
...
It does **not** turn Caesar into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Caesar memory
...
### 1) "I want Caesar to own a real phone number"
Use **Twilio**.
...
- pairs well with Caesar `text_to_speech` plus a public file host or tunnel
...
```bash
SCRIPT="$(find ~/.caesar/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
caesar skills search telephony
caesar skills install official/productivity/telephony
```
...
Then save credentials into Caesar:
...
3. Buy it and save it into `~/.caesar/.env` + state:
```bash
python3 "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python3 "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Caesar calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Caesar's existing `text_to_speech` support.

Use this when:
- you want the call to use Caesar's configured TTS voice rather than Twilio `<Say>`
...
Recommended Caesar TTS -> Twilio Play workflow:

1. Generate the audio with Caesar `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Caesar to create the message audio with `text_to_speech`
...
Important note:
- Caesar TTS is great for prerecorded outbound messages
...
- Caesar STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Caesar memory.
...
- Do not store arbitrary third-party phone numbers in Caesar memory.
...
3. persist that number to `~/.caesar/.env`
