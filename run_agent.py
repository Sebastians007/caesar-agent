...
_loaded_env_paths = load_caesar_dotenv(caesar_home=_caesar_home, project_env=_project_env)
...
class _SafeWriter:
    ...
    When caesar-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        # Caesar is a tool-using client by default.
        ...
        # Persistent error log -- always writes WARNING+ to ~/.caesar/logs/errors.log
        ...
        error_log_dir = _caesar_home / "logs"
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://github.com/Sebastians007/caesar-agent",
                        "X-OpenRouter-Title": "Caesar Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from caesar_cli.models import copilot_default_headers
                ...
                if _routed_client is not None:
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        raise RuntimeError(
                            ...
                            f"variable, or switch to a different provider with `caesar model`."
                        )
        ...
        # Session logs go into ~/.caesar/sessions/ alongside gateway sessions
        caesar_home = get_caesar_home()
        self.logs_dir = caesar_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("CAESAR_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from caesar_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from caesar_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from caesar_constants import get_caesar_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                            "platform": platform or "cli",
                            "caesar_home": str(_ghh()),
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "caesar"
                        except Exception:
                            pass
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("CAESAR_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the caesar-agent install
        ...
        from caesar_time import now as _caesar_now
        now = _caesar_now()
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from caesar_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("CAESAR_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("CAESAR_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            import httpx as _httpx
            _base_timeout = float(os.getenv("CAESAR_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("CAESAR_STREAM_READ_TIMEOUT", 60.0))
            stream_kwargs = {
                **api_kwargs,
                "stream": True,
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("CAESAR_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("CAESAR_STREAM_STALE_TIMEOUT", 180.0))
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("CAESAR_API_TIMEOUT", 1800.0)),
        }
        if self.tools:
        ...
        if _is_nous:
            extra_body["tags"] = ["product=caesar-agent"]
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from caesar_cli.models import github_model_reasoning_efforts
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=caesar-agent"]
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if env_var_enabled("CAESAR_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Caesar-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Clear stale keys: caesar config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Legacy cleanup: caesar config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if status_code in (401, 403) or "unauthorized" in error_msg or "forbidden" in error_msg or "permission" in error_msg:
                            ...
                            self._vprint(f"{self.log_prefix}      • Is the key valid? Run: caesar setup", force=True)
                        ...
                        if self._empty_content_retries < 3:
                            ...
                            if empty_response_info["is_local_custom"]:
                                error_message = (
                                    ...
                                    "Likely causes: wrong /v1 endpoint, runtime context window smaller than Caesar expects, "
