# OpenAI-Compatible API Server for Caesar Agent
...
SSE streaming. By exposing this endpoint, caesar-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  caesar-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.caesar/.env`
2. Run `caesar gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with caesar-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns caesar-agent as a model) |
...
Authorization: Bearer caesar-api-key-here
...
But caesar-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "caesar-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured caesar-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of caesar-agent's core prompt
...
Once implemented, caesar-agent works as a drop-in backend for:
