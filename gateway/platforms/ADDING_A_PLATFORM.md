...
Checklist for integrating a new messaging platform into the Caesar gateway.
...
And add it to the `caesar-gateway` composite:

```python
"caesar-gateway": {
    "includes": [..., "caesar-your-platform"]
}
```
...
## 12. Status Display (`caesar_cli/status.py`)
...
## 13. Gateway Setup Wizard (`caesar_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ caesar_cli/ toolsets.py \
  --include="*.py" -l | sort -u
