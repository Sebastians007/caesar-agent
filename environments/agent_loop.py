"""
CaesarAgentLoop -- Reusable Multi-Turn Agent Engine

Runs the caesar-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to caesar-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by CaesarAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by CaesarAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class CaesarAgentLoop:
    """
    Runs caesar-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # caesar-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("caesar")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via caesar-agent's dispatch
