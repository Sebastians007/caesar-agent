"""
CaesarAgentBaseEnv -- Abstract Base Environment for Caesar-Agent + Atropos

Provides the Atropos integration plumbing that all caesar-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via CaesarAgentLoop
...
# Ensure the caesar-agent repo root is on sys.path so that imports like
...
# Load API keys from caesar-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, CaesarAgentLoop
from environments.tool_context import ToolContext

# Import caesar-agent toolset infrastructure
...
class CaesarAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for caesar-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of caesar toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: caesar, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
...
class CaesarAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for caesar-agent Atropos integration.
    ...
    name: Optional[str] = "caesar-agent"
    env_config_cls = CaesarAgentEnvConfig

    def __init__(
        self,
        config: CaesarAgentEnvConfig,
        ...
        # Set terminal environment variables so caesar tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY caesar-agent tool (terminal, file, web,
