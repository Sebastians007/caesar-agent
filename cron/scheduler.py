...
Uses a file-based lock (~/.caesar/cron/.tick.lock) so only one tick
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_caesar_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    if origin:
        os.environ["CAESAR_SESSION_PLATFORM"] = origin["platform"]
        os.environ["CAESAR_SESSION_CHAT_ID"] = str(origin["chat_id"])
        if origin.get("chat_name"):
            os.environ["CAESAR_SESSION_CHAT_NAME"] = origin["chat_name"]
    ...
    try:
        ...
        try:
            load_dotenv(str(_caesar_home / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_caesar_home / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            os.environ["CAESAR_CRON_AUTO_DELIVER_PLATFORM"] = delivery_target["platform"]
            os.environ["CAESAR_CRON_AUTO_DELIVER_CHAT_ID"] = str(delivery_target["chat_id"])
            if delivery_target.get("thread_id") is not None:
                os.environ["CAESAR_CRON_AUTO_DELIVER_THREAD_ID"] = str(delivery_target["thread_id"])

        model = job.get("model") or os.getenv("CAESAR_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_caesar_home / "config.yaml")
        ...
        prefill_file = os.getenv("CAESAR_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            import json as _json
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _caesar_home / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            runtime_kwargs = {
                "requested": job.get("provider") or os.getenv("CAESAR_INFERENCE_PROVIDER"),
            }
            if job.get("base_url"):
        ...
        _cron_timeout = float(os.getenv("CAESAR_CRON_TIMEOUT", 600))
...
**Job ID:** {job_id}
**Run Time:** {_caesar_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        for key in (
            "CAESAR_SESSION_PLATFORM",
            "CAESAR_SESSION_CHAT_ID",
            "CAESAR_SESSION_CHAT_NAME",
            "CAESAR_CRON_AUTO_DELIVER_PLATFORM",
            "CAESAR_CRON_AUTO_DELIVER_CHAT_ID",
            "CAESAR_CRON_AUTO_DELIVER_THREAD_ID",
        ):
...
def tick(verbose: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _caesar_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _caesar_now().strftime('%H:%M:%S'), len(due_jobs))
