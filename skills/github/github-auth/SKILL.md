...
- Give it a name like "caesar-agent"
- Select scopes:
...
- Give it a title like "caesar-agent-<machine-name>"
...
elif [ -f ~/.caesar/.env ] && grep -q "^GITHUB_TOKEN=" ~/.caesar/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.caesar/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
