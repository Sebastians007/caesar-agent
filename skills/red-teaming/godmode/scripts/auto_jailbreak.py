...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.caesar/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("CAESAR_HOME", Path.home() / ".caesar")) / "skills" / "red-teaming" / "godmode"
...
CAESAR_HOME = Path(os.getenv("CAESAR_HOME", Path.home() / ".caesar"))
CONFIG_PATH = CAESAR_HOME / "config.yaml"
PREFILL_PATH = CAESAR_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Caesar models — already uncensored, just needs clean prompt
    "caesar": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "caesar" in model_lower or "nous" in model_lower:
        return "caesar"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Caesar config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.caesar/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Caesar for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Caesar for changes to take effect.")
