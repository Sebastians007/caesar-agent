# Contributing to Caesar Agent

Thank you for contributing to Caesar Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — Windows, macOS, different Linux distros, different terminal emulators. We want Caesar to work everywhere.
...
Bundled skills (in `skills/`) ship with every Caesar install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `caesar skills browse` (labeled "official") and install it with `caesar skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `caesar skills install`.
...
```bash
git clone --recurse-submodules https://github.com/Sebastians007/caesar-agent.git
cd caesar-agent
...
```bash
mkdir -p ~/.caesar/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.caesar/config.yaml
touch ~/.caesar/.env

# Add at minimum an LLM provider key:
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.caesar/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/caesar" ~/.local/bin/caesar
...
├── cli.py                    # CaesarCLI class — interactive TUI, prompt_toolkit integration
├── model_tools.py            # Tool orchestration (thin layer over tools/registry.py)
├── toolsets.py               # Tool groupings and presets (caesar-cli, caesar-telegram, etc.)
├── caesar_state.py           # SQLite session database with FTS5 full-text search, session titles
...
│
├── caesar_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.caesar/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.caesar/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.caesar/`)

| Path | Purpose |
|------|---------|
| `~/.caesar/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.caesar/.env` | API keys and secrets |
| `~/.caesar/auth.json` | OAuth credentials (Nous Portal) |
| `~/.caesar/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.caesar/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.caesar/state.db` | SQLite session database |
| `~/.caesar/sessions/` | JSON session logs |
| `~/.caesar/cron/` | Scheduled job data |
| `~/.caesar/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`caesar_state.py`) with full-text search and unique session titles. JSON logs go to `~/.caesar/sessions/`.
...
Four fields are supported under `metadata.caesar`:
...
The user may skip setup and keep loading the skill. Caesar only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `caesar setup` or update `~/.caesar/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Caesar tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `caesar --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Caesar uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.caesar/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `caesar_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `caesar_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Caesar runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
Caesar has terminal access. Security matters.
...
2. **Test manually**: Run `caesar` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/Sebastians007/caesar-agent/issues)
- Include: OS, Python version, Caesar version (`caesar version`), full error traceback
