...
## Token Lifecycle
- Store at `~/.caesar/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `caesar_cli/auth.py` — Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `caesar_cli/models.py` — Add Gemini model catalog
4. `caesar_cli/runtime_provider.py` — Add gemini branch (read OAuth token, build OpenAI client)
5. `caesar_cli/main.py` — Add `_model_flow_gemini()`, add to provider choices
6. `caesar_cli/setup.py` — Add gemini auth flow (trigger browser OAuth)
...
- OR: accept user-provided client_id via CAESAR_GEMINI_CLIENT_ID env var
...
- caesar-agent Copilot OAuth: `caesar_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
