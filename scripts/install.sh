...
# Configuration
REPO_URL_SSH="git@github.com:Sebastians007/caesar-agent.git"
REPO_URL_HTTPS="https://github.com/Sebastians007/caesar-agent.git"
CAESAR_HOME="$HOME/.caesar"
INSTALL_DIR="${CAESAR_INSTALL_DIR:-$CAESAR_HOME/caesar-agent}"
PYTHON_VERSION="3.11"
NODE_VERSION="22"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.caesar/caesar-agent)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/Sebastians007/caesar-agent/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
check_node() {
    ...
    if [ -x "$CAESAR_HOME/node/bin/node" ]; then
        export PATH="$CAESAR_HOME/node/bin:$PATH"
        local found_ver=$("$CAESAR_HOME/node/bin/node" --version)
        log_success "Node.js $found_ver found (Caesar-managed)"
        HAS_NODE=true
        return 0
    fi
...
install_node() {
    ...
    log_info "Extracting to ~/.caesar/node/..."
    ...
    # Place into ~/.caesar/node/ and symlink binaries to ~/.local/bin/
    rm -rf "$CAESAR_HOME/node"
    mkdir -p "$CAESAR_HOME"
    mv "$extracted_dir" "$CAESAR_HOME/node"
    rm -rf "$tmp_dir"

    mkdir -p "$HOME/.local/bin"
    ln -sf "$CAESAR_HOME/node/bin/node" "$HOME/.local/bin/node"
    ln -sf "$CAESAR_HOME/node/bin/npm"  "$HOME/.local/bin/npm"
    ln -sf "$CAESAR_HOME/node/bin/npx"  "$HOME/.local/bin/npx"

    export PATH="$CAESAR_HOME/node/bin:$PATH"

    local installed_ver
    installed_ver=$("$CAESAR_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.caesar/node/"
    HAS_NODE=true
}
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Caesar Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                local stash_name
                stash_name="caesar-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Caesar behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up caesar command..."

    if [ "$USE_VENV" = true ]; then
        CAESAR_BIN="$INSTALL_DIR/venv/bin/caesar"
    else
        CAESAR_BIN="$(which caesar 2>/dev/null || echo "")"
        if [ -z "$CAESAR_BIN" ]; then
            log_warn "caesar not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$CAESAR_BIN" ]; then
        log_warn "caesar entry point not found at $CAESAR_BIN"
    ...
    mkdir -p "$HOME/.local/bin"
    ln -sf "$CAESAR_BIN" "$HOME/.local/bin/caesar"
    log_success "Symlinked caesar → ~/.local/bin/caesar"
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Caesar Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
    ...
    # Export for current session so caesar works immediately
    export PATH="$HOME/.local/bin:$PATH"

    log_success "caesar command ready"
}

copy_config_templates() {
    log_info "Setting up configuration files..."

    # Create ~/.caesar directory structure (config at top level, code in subdir)
    mkdir -p "$CAESAR_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills,whatsapp/session}

    # Create .env at ~/.caesar/.env (top level, easy to find)
    if [ ! -f "$CAESAR_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$CAESAR_HOME/.env"
            log_success "Created ~/.caesar/.env from template"
        else
            touch "$CAESAR_HOME/.env"
            log_success "Created ~/.caesar/.env"
        fi
    else
        log_info "~/.caesar/.env already exists, keeping it"
    fi

    # Create config.yaml at ~/.caesar/config.yaml (top level, easy to find)
    if [ ! -f "$CAESAR_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$CAESAR_HOME/config.yaml"
            log_success "Created ~/.caesar/config.yaml from template"
        fi
    else
        log_info "~/.caesar/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$CAESAR_HOME/SOUL.md" ]; then
        cat > "$CAESAR_HOME/SOUL.md" << 'SOUL_EOF'
# Caesar Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how Caesar communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.caesar/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.caesar/"

    # Seed bundled skills into ~/.caesar/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.caesar/skills/ ..."
    if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
        log_success "Skills synced to ~/.caesar/skills/"
    else
        # Fallback: simple directory copy if Python sync fails
        if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$CAESAR_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
            cp -r "$INSTALL_DIR/skills/"* "$CAESAR_HOME/skills/" 2>/dev/null || true
            log_success "Skills copied to ~/.caesar/skills/"
        fi
    fi
}
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        case "$DISTRO" in
            ...
            *)
                ...
                log_info "This is standard Playwright setup — Caesar itself does not require root access."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'caesar setup' after install."
        return 0
    fi
    ...
    # Run caesar setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m caesar_cli.main setup < /dev/tty
    else
        python -m caesar_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$CAESAR_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Caesar to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$CAESAR_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'caesar whatsapp' to pair via QR code..."
            echo ""
            read -p "Pair WhatsApp now? [Y/n] " -n 1 -r
            echo
            if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
                CAESAR_CMD="$HOME/.local/bin/caesar"
                [ ! -x "$CAESAR_CMD" ] && CAESAR_CMD="caesar"
                $CAESAR_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'caesar whatsapp' to pair."
        fi
    fi

    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'caesar gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        CAESAR_CMD="$HOME/.local/bin/caesar"
        if [ ! -x "$CAESAR_CMD" ]; then
            CAESAR_CMD="caesar"
        fi

        if command -v systemctl &> /dev/null; then
            log_info "Installing systemd service..."
            if $CAESAR_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $CAESAR_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: caesar gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: caesar gateway"
            fi
        else
            log_info "systemd not available — starting gateway in background..."
            nohup $CAESAR_CMD gateway > "$CAESAR_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.caesar/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: caesar gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: caesar gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.caesar/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.caesar/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.caesar/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.caesar/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.caesar/caesar-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}caesar${NC}              Start chatting"
    echo -e "   ${GREEN}caesar setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}caesar config${NC}       View/edit configuration"
    echo -e "   ${GREEN}caesar config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}caesar gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}caesar update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'caesar' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
