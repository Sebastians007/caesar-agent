...
VERSION_FILE = REPO_ROOT / "caesar_cli" / "__init__.py"
...
AUTHOR_MAP = {
    ...
    "sebastian@sebastians007.com": "Sebastians007",
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/Sebastians007/caesar-agent",
    ...
    lines.append(f"# Caesar Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Caesar Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Caesar Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Caesar Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Caesar Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"Caesar Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'Caesar Agent v{new_version} ({calver_date})' "
