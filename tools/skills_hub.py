#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Caesar Skills Hub.
...
Used by caesar_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Sebastian Sartele) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                caesar_meta = meta_block.get("caesar", {})
                if isinstance(caesar_meta, dict):
                    tags = caesar_meta.get("tags", [])
