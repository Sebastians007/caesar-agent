...
it is automatically downloaded from GitHub releases to $CAESAR_HOME/bin/tirith.
...
def _get_caesar_home() -> str:
    """Return the Caesar home directory, respecting CAESAR_HOME env var."""
    return str(get_caesar_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_caesar_home(), ".tirith-install-failed")
...
def _caesar_bin_dir() -> str:
    """Return $CAESAR_HOME/bin, creating it if needed."""
    d = os.path.join(_get_caesar_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $CAESAR_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_caesar_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $CAESAR_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $CAESAR_HOME/bin/tirith
    ...
    caesar_bin = os.path.join(_caesar_bin_dir(), "tirith")
    if os.path.isfile(caesar_bin) and os.access(caesar_bin, os.X_OK):
        _resolved_path = caesar_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return caesar_bin
