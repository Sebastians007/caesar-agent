...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Caesar exit
...
# display_caesar_home imported lazily at call site (stale-module safety during caesar update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of caesar directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "caesar-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("CAESAR_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (CAESAR_INTERACTIVE=1).
    ...
    try:
        os.environ["CAESAR_SPINNER_PAUSE"] = "1"
        ...
        if "CAESAR_SPINNER_PAUSE" in os.environ:
            del os.environ["CAESAR_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str) -> tuple[str, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (CAESAR_INTERACTIVE=1):
    ...
    if not sudo_password:
        ...
        if os.getenv("CAESAR_INTERACTIVE"):
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.caesar/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "CAESAR_ENABLE_NOUS_MANAGED_TOOLS is not enabled and no direct "
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            session_key = os.getenv("CAESAR_SESSION_KEY", "")
            ...
            try:
                ...
                if check_interval and background:
                    ...
                    watcher_platform = os.getenv("CAESAR_SESSION_PLATFORM", "")
                    watcher_chat_id = os.getenv("CAESAR_SESSION_CHAT_ID", "")
                    watcher_thread_id = os.getenv("CAESAR_SESSION_THREAD_ID", "")
