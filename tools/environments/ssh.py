...
class SSHEnvironment(PersistentShellMixin, BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "caesar-ssh"
    ...
    def _sync_skills_and_credentials(self) -> None:
        ...
        try:
            container_base = f"{self._remote_home}/.caesar"
            ...
            # Sync individual credential files (remap /root/.caesar to detected home)
            for mount_entry in get_credential_file_mounts():
                remote_path = mount_entry["container_path"].replace("/root/.caesar", container_base, 1)
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/caesar-ssh-{self._session_id}"
    ...
    def _read_temp_files(self, *paths: str) -> list[str]:
        ...
        delim = f"__CAESAR_SEP_{self._session_id}__"
        script = "; ".join(
