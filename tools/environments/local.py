...
_OUTPUT_FENCE = "__CAESAR_FENCE_a9f7b3__"

# Caesar-internal env vars that should NOT leak into terminal subprocesses.
# These are loaded from ~/.caesar/.env for Caesar' own LLM/provider calls
# but can break external CLIs (e.g. codex) that also honor them.
# See: https://github.com/Sebastians007/caesar-agent/issues/1002
#
...
_CAESAR_PROVIDER_ENV_FORCE_PREFIX = "_CAESAR_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    optional config registry, so new Caesar-managed secrets are blocked
    ...
    try:
        from caesar_cli.config import OPTIONAL_ENV_VARS
    ...
    # Vars not covered above but still Caesar-internal / conflict-prone.
    blocked.update({
        "OPENAI_BASE_URL",
        "OPENAI_API_KEY",
...
_CAESAR_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Caesar-managed secrets from a subprocess environment.

    `_CAESAR_FORCE_<VAR>` entries in ``extra_env`` opt a blocked variable back in
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_CAESAR_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _CAESAR_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_CAESAR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_CAESAR_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _CAESAR_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("CAESAR_GIT_BASH_PATH")
    ...
    raise RuntimeError(
        "Git Bash not found. Caesar Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set CAESAR_GIT_BASH_PATH to your bash.exe location."
    )
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_CAESAR_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_CAESAR_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _CAESAR_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
...
class LocalEnvironment(PersistentShellMixin, BaseEnvironment):
    ...
    def _temp_prefix(self) -> str:
        return f"/tmp/caesar-local-{self._session_id}"
    ...
    def _execute_oneshot(self, command: str, cwd: str = "", *,
        ...
        user_shell = _find_bash()
        # Newline-separated wrapper (not `cmd; __caesar_rc=...` on one line).
        # A trailing `; __caesar_rc` glued to `<<EOF` / a closing `EOF` line breaks
