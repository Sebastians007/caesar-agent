...
wrapper, while preserving Caesar' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_caesar_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseModalExecutionEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("caesar-agent", create_if_missing=True)
