...
def _load_caesar_env_vars() -> dict[str, str]:
    """Load ~/.caesar/.env values without failing Docker command execution."""
    try:
        from caesar_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.caesar/sandboxes/). Non-persistent
        ...
        container_name = f"caesar-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def execute(self, command: str, cwd: str = "", *,
        ...
        try:
            ...
            pass
        caesar_env = _load_caesar_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = caesar_env.get(key)
