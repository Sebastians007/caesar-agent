...
Lets the LLM write a Python script that calls Caesar tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture:
  1. Parent generates a `caesar_tools.py` stub module with RPC functions
...
# ---------------------------------------------------------------------------
# caesar_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_caesar_tools_module(enabled_tools: List[str]) -> str:
    """
    Build the source code for the caesar_tools.py stub module.
    ...
    header = '''\
"""Auto-generated Caesar tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["CAESAR_RPC_SOCKET"])
...
def execute_code(
    ...
    # --- Set up temp directory with caesar_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="caesar_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"caesar_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated caesar_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_caesar_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "caesar_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        for k, v in os.environ.items():
            ...
            if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
                child_env[k] = v
        child_env["CAESAR_RPC_SOCKET"] = sock_path
        child_env["PYTHONDONTWRITEBYTECODE"] = "1"
        # Ensure the caesar-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.
        _caesar_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        child_env["PYTHONPATH"] = _caesar_root + (os.pathsep + _existing_pp if _existing_pp else "")
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.
        _tz_name = os.getenv("CAESAR_TIMEZONE", "").strip()
        if _tz_name:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.caesar/.env')).
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``caesar tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Caesar tools programmatically. "
        ...
        f"Available via `from caesar_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into caesar_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from caesar_tools import {import_str}` "
