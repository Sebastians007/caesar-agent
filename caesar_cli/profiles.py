"""
Profile management for multiple isolated Caesar instances.

Each profile is a fully independent CAESAR_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.caesar/profiles/<name>/`` by default.

The "default" profile is ``~/.caesar`` itself — backward compatible,
zero migration needed.

Usage::

    caesar profile create coder          # fresh profile + bundled skills
    caesar profile create coder --clone  # also copy config, .env, SOUL.md
    caesar profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    caesar -p coder chat                 # or via flag
    caesar profile use coder             # set as sticky default
    caesar profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.caesar) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "caesar-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "caesar", "default", "test", "tmp", "root", "sudo",
})

# Caesar subcommands that cannot be used as profile names/aliases
_CAESAR_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.caesar/profiles/`` — anchored to the user's home,
    NOT to the current CAESAR_HOME (which may itself be a profile).
    This ensures ``coder profile list`` can see all profiles.
    """
    return Path.home() / ".caesar" / "profiles"


def _get_default_caesar_home() -> Path:
    """Return the default (pre-profile) CAESAR_HOME path."""
    return Path.home() / ".caesar"


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_caesar_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.caesar
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its CAESAR_HOME directory."""
    if name == "default":
        return _get_default_caesar_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, caesar subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _CAESAR_SUBCOMMANDS:
        return f"'{name}' conflicts with a caesar subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_caesar_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.caesar)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches CAESAR_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "CAESAR_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.caesar).\n"
            "To remove everything, use: caesar uninstall"
        )
    ...
    # Check for service
    from caesar_cli.gateway import _profile_suffix, get_service_name
    wrapper_path = _get_wrapper_dir() / name
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set CAESAR_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("CAESAR_HOME")
    try:
        os.environ["CAESAR_HOME"] = str(profile_dir)
        from caesar_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["CAESAR_HOME"] = old_home
        elif "CAESAR_HOME" in os.environ:
            del os.environ["CAESAR_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.caesar/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: caesar profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from CAESAR_HOME.

    Returns ``"default"`` if CAESAR_HOME is not set or points to ``~/.caesar``.
    Returns the profile name if CAESAR_HOME points into ``~/.caesar/profiles/<name>``.
    Returns ``"custom"`` if CAESAR_HOME is set to an unrecognized path.
    """
    from caesar_constants import get_caesar_home
    caesar_home = get_caesar_home()
    resolved = caesar_home.resolve()

    default_resolved = _get_default_caesar_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.caesar itself — its parent is ~/ and its
        # directory name is ".caesar", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: caesar profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.caesar itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.caesar). "
            "Specify a different name: caesar profile import <archive> --name <name>"
        )
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for caesar profile names."""
    return '''# Caesar Agent profile completion
# Add to ~/.bashrc: eval "$(caesar completion bash)"

_caesar_profiles() {
    local profiles_dir="$HOME/.caesar/profiles"
...
_caesar_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_caesar_profiles)" -- "$cur"))
        return
    fi
...
complete -F _caesar_completion caesar
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for caesar profile names."""
    return '''#compdef caesar
# Caesar Agent profile completion
# Add to ~/.zshrc: eval "$(caesar completion zsh)"

_caesar() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.caesar/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.caesar/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a CAESAR_HOME path string.

    Called early in the CLI entry point, before any caesar modules
    are imported, to set the CAESAR_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: caesar profile create {profile_name}"
        )
