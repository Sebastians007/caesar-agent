"""caesar webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.caesar/webhook_subscriptions.json and are
...
def _caesar_home() -> Path:
    return Path(
        os.getenv("CAESAR_HOME", str(Path.home() / ".caesar"))
    ).expanduser()


def _subscriptions_path() -> Path:
    return _caesar_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: caesar gateway run
"""
...
def webhook_command(args):
    """Entry point for 'caesar webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: caesar webhook {subscribe|list|remove|test}")
        print("Run 'caesar webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (caesar gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: caesar webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from caesar webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (caesar gateway run)")
