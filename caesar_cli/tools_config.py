"""
Unified tool configuration for Caesar Agent.

`caesar tools` and `caesar setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.caesar/config.yaml under
...
# They're still in _CAESAR_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from caesar_cli.plugins import discover_plugins, get_plugin_toolsets
...
PLATFORMS = {
    "cli":      {"label": "🖥️  CLI",       "default_toolset": "caesar-cli"},
    "telegram": {"label": "📱 Telegram",   "default_toolset": "caesar-telegram"},
    "discord":  {"label": "💬 Discord",    "default_toolset": "caesar-discord"},
    "slack":    {"label": "💼 Slack",      "default_toolset": "caesar-slack"},
    "whatsapp": {"label": "📱 WhatsApp",   "default_toolset": "caesar-whatsapp"},
    "signal":   {"label": "📡 Signal",     "default_toolset": "caesar-signal"},
    "homeassistant": {"label": "🏠 Home Assistant", "default_toolset": "caesar-homeassistant"},
    "email":    {"label": "📧 Email",      "default_toolset": "caesar-email"},
    "matrix":   {"label": "💬 Matrix",     "default_toolset": "caesar-matrix"},
 "dingtalk": {"label": "💬 DingTalk", "default_toolset": "caesar-dingtalk"},
    "feishu": {"label": "🪽 Feishu", "default_toolset": "caesar-feishu"},
    "wecom": {"label": "💬 WeCom", "default_toolset": "caesar-wecom"},
    "api_server": {"label": "🌐 API Server", "default_toolset": "caesar-api-server"},
    "mattermost": {"label": "💬 Mattermost", "default_toolset": "caesar-mattermost"},
    "webhook": {"label": "🔗 Webhook", "default_toolset": "caesar-webhook"},
}
    ...
    if post_setup_key == "browserbase":
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_caesar_home()}/caesar-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in caesar-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "caesar-cli" (which include all _CAESAR_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "caesar-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `caesar tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by caesar tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (caesar-cli, caesar-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.caesar/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `caesar tools` and `caesar setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Caesar Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from caesar_constants import display_caesar_home
    print(color(f"  Tool configuration saved to {display_caesar_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'caesar' or gateway restart.", Colors.DIM))
    print()
