"""
MCP Server Management CLI — ``caesar mcp`` subcommand.

Implements ``caesar mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.caesar/config.yaml under the ``mcp_servers`` key.
"""
...
from caesar_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_caesar_home,  # noqa: F401 — used by test mocks
)
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  caesar mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  caesar mcp add github --command npx --args @modelcontextprotocol/server-github')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_caesar_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: caesar mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_caesar_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  caesar mcp add <name> --url <endpoint>')
        _info('  caesar mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
# ─── caesar mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'caesar mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``caesar mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("caesar mcp serve                              Run as MCP server")
        _info("caesar mcp add <name> --url <endpoint>        Add an MCP server")
        _info("caesar mcp add <name> --command <cmd>         Add a stdio server")
        _info("caesar mcp remove <name>                      Remove a server")
        _info("caesar mcp list                               List servers")
        _info("caesar mcp test <name>                        Test connection")
        _info("caesar mcp configure <name>                   Toggle tools")
        print()
