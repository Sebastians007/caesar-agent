"""Slash command definitions and autocomplete for the Caesar CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update Caesar Agent to the latest version", "Info",
               gateway_only=True),
...
def _resolve_config_gates() -> set[str]:
    ...
    try:
        ...
        config_path = os.path.join(
            os.getenv("CAESAR_HOME", os.path.expanduser("~/.caesar")),
            "config.yaml",
        )
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /caesar handler.

    Maps both canonical names and aliases so /caesar bg do stuff works
    the same as /caesar background do stuff.
    """
    overrides = _resolve_config_gates()
