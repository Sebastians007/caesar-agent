"""
Caesar Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   – ``~/.caesar/plugins/<name>/``
2. **Project plugins** – ``./.caesar/plugins/<name>/`` (opt-in via
   ``CAESAR_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     – packages that expose the ``caesar_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "caesar_agent.plugins"

_NS_PARENT = "caesar_plugins"
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.caesar/plugins/)
        caesar_home = os.environ.get("CAESAR_HOME", os.path.expanduser("~/.caesar"))
        user_dir = Path(caesar_home) / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.caesar/plugins/)
        if _env_enabled("CAESAR_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".caesar" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``caesar_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``caesar tools`` TUI so plugin-provided toolsets appear
