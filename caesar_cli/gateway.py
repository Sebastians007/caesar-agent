"""
Gateway subcommand for caesar CLI.

Handles: caesar gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from caesar_cli.config import get_env_value, get_caesar_home, save_env_value, is_managed, managed_error
# display_caesar_home is imported lazily at call sites to avoid ImportError
# when caesar_constants is cached from a pre-update version during `caesar update`.
from caesar_cli.setup import (
...
def find_gateway_pids() -> list:
    ...
    pids = []
    patterns = [
        "caesar_cli.main gateway",
        "caesar_cli/main.py gateway",
        "caesar gateway",
        "gateway/run.py",
    ]
...
_SERVICE_BASE = "caesar-gateway"
SERVICE_DESCRIPTION = "Caesar Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current CAESAR_HOME.

    Returns ``""`` for the default ``~/.caesar``, the profile name for
    ``~/.caesar/profiles/<name>``, or a short hash for any other custom
    CAESAR_HOME path.
    """
    import hashlib
    import re
    from pathlib import Path as _Path
    home = get_caesar_home().resolve()
    default = (_Path.home() / ".caesar").resolve()
    if home == default:
        return ""
    # Detect ~/.caesar/profiles/<name> pattern → use the profile name
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary CAESAR_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def get_service_name() -> str:
    """Derive a systemd service name scoped to this CAESAR_HOME.

    Default ``~/.caesar`` returns ``caesar-gateway`` (backward compatible).
    Profile ``~/.caesar/profiles/coder`` returns ``caesar-gateway-coder``.
    Any other CAESAR_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    caesar gateway uninstall")
    print_info("    sudo caesar gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Caesar can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo caesar gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo caesar gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo caesar gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.caesar`` → ``ai.caesar.gateway.plist`` (backward compatible).
    Profile ``~/.caesar/profiles/coder`` → ``ai.caesar.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.caesar.gateway-{suffix}" if suffix else "ai.caesar.gateway"
...
def get_caesar_cli_path() -> str:
    """Get the path to the caesar CLI."""
    ...
    return f"{get_python_path()} -m caesar_cli.main"
...
def _caesar_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current CAESAR_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_caesar_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.caesar                    → /home/alice/.caesar
      /root/.caesar/profiles/coder     → /home/alice/.caesar/profiles/coder
      /opt/custom-caesar               → /opt/custom-caesar  (kept as-is)
    """
    current_caesar = get_caesar_home().resolve()
    current_default = (Path.home() / ".caesar").resolve()
    target_default = Path(target_home_dir) / ".caesar"

    # Default ~/.caesar → remap to target user's default
    if current_caesar == current_default:
        return str(target_default)

    # Profile or subdir of ~/.caesar → preserve the relative structure
    try:
        relative = current_caesar.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.caesar) — keep as-is
        return str(current_caesar)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        caesar_home = _caesar_home_for_target_user(home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m caesar_cli.main gateway run --replace
...
Environment="CAESAR_HOME={caesar_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    caesar_home = str(get_caesar_home().resolve())
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Caesar install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}caesar gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}caesar gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}caesar gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}caesar gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}caesar gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.caesar.gateway-{suffix}" if suffix else "ai.caesar.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current Caesar install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  caesar gateway status             # Check status")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Caesar install")
    else:
        print("⚠ Service definition is stale relative to the current Caesar install")
        print("  Run: caesar gateway start")
    ...
    if deep:
        log_file = get_caesar_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Caesar will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @caesar:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. caesar) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Caesar delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Caesar agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Caesar will use (e.g., caesar@gmail.com)."},
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_caesar_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'caesar pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: caesar pairing approve {platform} {code}")
                else:
                    print_info("  Skipped — configure later with 'caesar gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from caesar_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"CaesarAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: caesar gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: caesar gateway")
    else:
        print()
        print_info("No platforms configured. Run 'caesar gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: caesar gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: caesar gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  caesar gateway install")
                print("  sudo caesar gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  caesar gateway          # Run in foreground")
                print("  caesar gateway install  # Install as user service")
                print("  sudo caesar gateway install --system  # Install as boot-time system service")
