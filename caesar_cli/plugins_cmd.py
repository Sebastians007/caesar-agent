"""``caesar plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.caesar/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    caesar_home = os.environ.get("CAESAR_HOME", os.path.expanduser("~/.caesar"))
    plugins = Path(caesar_home) / "plugins"
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]caesar plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid Caesar plugin."
        )
    ...
    console.print("[dim]  caesar gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] caesar plugins install owner/repo")
        return
    ...
    for d in dirs:
        ...
        # Check if it's a git repo (installed via caesar plugins install)
        if (d / ".git").exists():
            source = "git"
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] caesar plugins")
    console.print("[dim]Enable/disable:[/dim] caesar plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch caesar plugins subcommands."""
