...
from caesar_cli.auth import get_nous_auth_status
from caesar_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: caesar setup tools, caesar setup terminal, or caesar status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `caesar setup` and `caesar model`."""
    if not managed_nous_tools_enabled():
        return set()
