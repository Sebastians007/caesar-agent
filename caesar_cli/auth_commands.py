...
import caesar_cli.auth as auth_mod
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_caesar_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:caesar_pkce",
    ...
    raise SystemExit(f"`caesar auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `caesar auth` is called bare."""
