#!/usr/bin/env python3
"""
Caesar CLI - Main entry point.

Usage:
    caesar                     # Interactive chat (default)
    caesar chat                # Interactive chat
    caesar gateway             # Run gateway in foreground
    caesar gateway start       # Start gateway as service
    caesar gateway stop        # Stop gateway service
    caesar gateway status      # Show gateway status
    caesar gateway install     # Install gateway service
    caesar gateway uninstall   # Uninstall gateway service
    caesar setup               # Interactive setup wizard
    caesar logout              # Clear stored authentication
    caesar status              # Show status of all components
    caesar cron                # Manage cron jobs
    caesar cron list           # List cron jobs
    caesar cron status         # Check if cron scheduler is running
    caesar doctor              # Check configuration and dependencies
    caesar honcho setup                    # Configure Honcho AI memory integration
    caesar honcho status                   # Show Honcho config and connection status
    caesar honcho sessions                 # List directory → session name mappings
    caesar honcho map <name>               # Map current directory to a session name
    caesar honcho peer                     # Show peer names and dialectic settings
    caesar honcho peer --user NAME         # Set user peer name
    caesar honcho peer --ai NAME           # Set AI peer name
    caesar honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    caesar honcho mode                     # Show current memory mode
    caesar honcho mode [hybrid|honcho|local]  # Set memory mode
    caesar honcho tokens                   # Show token budget settings
    caesar honcho tokens --context N       # Set session.context() token cap
    caesar honcho tokens --dialectic N     # Set dialectic result char cap
    caesar honcho identity                 # Show AI peer identity representation
    caesar honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    caesar honcho migrate                  # Step-by-step migration guide: OpenClaw native → Caesar + Honcho
    caesar version             Show version
    caesar update              Update to latest version
    caesar uninstall           Uninstall Caesar Agent
    caesar acp                 Run as an ACP server for editor integration
    caesar sessions browse     Interactive session picker with search

    caesar claw migrate --dry-run  # Preview migration without changes
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (caesar tools, caesar setup, caesar model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'caesar {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any caesar module import.
#
# Many modules cache CAESAR_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("CAESAR_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.caesar/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set CAESAR_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.caesar/active_profile
    if profile_name is None:
        try:
            active_path = Path.home() / ".caesar" / "active_profile"
    ...
    # 3. If we found a profile, resolve and set CAESAR_HOME
    if profile_name is not None:
        try:
            from caesar_cli.profiles import resolve_profile_env
            caesar_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent caesar from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["CAESAR_HOME"] = caesar_home
...
# Load .env from ~/.caesar/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from caesar_cli.config import get_caesar_home
from caesar_cli.env_loader import load_caesar_dotenv
load_caesar_dotenv(project_env=PROJECT_ROOT / '.env')
...
from caesar_cli import __version__, __release_date__
from caesar_constants import OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from caesar_cli.config import get_env_path, get_caesar_home, load_config
    from caesar_cli.auth import get_auth_status

    # Determine whether Caesar itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from caesar_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_caesar_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from caesar_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_caesar_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Caesar has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Caesar to use their tokens.
    if _has_caesar_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'caesar sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Caesar isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  caesar setup")
        print()

        from caesar_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'caesar setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from caesar_cli.banner import prefetch_update_check
    ...
    if getattr(args, "source", None):
        os.environ["CAESAR_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from caesar_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from caesar_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Caesar?")
        print()
    ...
    session_dir = get_caesar_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: caesar gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  caesar gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Caesar Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  caesar gateway")
        ...
        print()
        print("  Or install as a service: caesar gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'caesar whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from caesar_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``caesar model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from caesar_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from caesar_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("CAESAR_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from caesar_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from caesar_cli.config import get_env_value, save_env_value
    ...
    from caesar_cli.models import model_ids
    ...
    if selected:
        ...
        from caesar_cli.config import load_config, save_config
        cfg = load_config()
        model = cfg.get("model")
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from caesar_cli.config import get_env_value, save_config, save_env_value
    from caesar_cli.nous_subscription import (
    ...
    from caesar_cli.models import _PROVIDER_MODELS
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from caesar_cli.codex_models import get_codex_model_ids
    from caesar_cli.config import get_env_value, save_env_value
    import argparse
    ...
    _codex_token = None
    try:
        from caesar_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_custom(config):
    ...
    """
    from caesar_cli.auth import _save_model_choice, deactivate_provider
    from caesar_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    from caesar_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `caesar model` to set a model.")
...
def _model_flow_named_custom(config, provider_info):
    ...
    """
    from caesar_cli.auth import _save_model_choice, deactivate_provider
    from caesar_cli.config import save_env_value, load_config, save_config
    from caesar_cli.models import fetch_api_models
...
def _model_flow_copilot(config, current_model=""):
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from caesar_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from caesar_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Caesar turns to `copilot --acp`.")
    print("  Caesar currently starts its own ACP subprocess for each request.")
    print("  Caesar uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print("  Set CAESAR_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from caesar_cli.config import get_env_value, save_env_value, load_config, save_config
    from caesar_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from caesar_constants import display_caesar_home as _dhh_fn
            print(f"    Caesar will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'caesar model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Caesar CLI with a provider."""
    from caesar_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from caesar_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from caesar_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from caesar_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from caesar_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from caesar_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from caesar_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from caesar_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"Caesar Agent v{__version__} ({__release_date__})")
    ...
    try:
        from caesar_cli.banner import check_for_updates
        from caesar_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Caesar Agent."""
    _require_tty("uninstall")
    from caesar_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _update_via_zip(args):
    """Update Caesar Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/Sebastians007/caesar-agent/archive/refs/heads/{branch}.zip"
    
    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="caesar-update-")
        zip_path = os.path.join(tmp_dir, f"caesar-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to caesar-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"caesar-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("caesar-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Caesar behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Caesar can run?")
    ...
    if stash_selector is None:
        print("⚠ Local changes were restored, but Caesar couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("⚠ Local changes were restored, but Caesar couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Caesar behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `caesar update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/Sebastians007/caesar-agent.git",
    "git@github.com:Sebastians007/caesar-agent.git",
    "https://github.com/Sebastians007/caesar-agent",
    "git@github.com:Sebastians007/caesar-agent",
}
OFFICIAL_REPO_URL = "https://github.com/Sebastians007/caesar-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_caesar_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from caesar_constants import get_caesar_home
        (get_caesar_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Caesar repository.")
        print("  This means you may miss updates from Sebastians007/caesar-agent.")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ✓ Added upstream: https://github.com/Sebastians007/caesar-agent.git")
                has_upstream = True
            else:
                print("  ✗ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/Sebastians007/caesar-agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``caesar update``, every profile is now current.
    """
    homes = []
    # Default profile home
    default_home = Path.home() / ".caesar"
    homes.append(default_home)
    # Named profiles under ~/.caesar/profiles/
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "caesar-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Caesar Agent to the latest version."""
    import shutil
    from caesar_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Caesar Agent")
        return
    
    print("⚕ Updating Caesar Agent...")
    print()
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/Sebastians007/caesar-agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        # the old bytecode (e.g. get_caesar_home added to caesar_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload caesar_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_caesar_home() added since the last release.
        try:
            import importlib
            import caesar_constants as _hc
        ...
        try:
            from caesar_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if not (sys.stdin.isatty() and sys.stdout.isatty()):
                ...
                print("    Run 'caesar config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                print()
                print("Skipped. Run 'caesar config migrate' later to configure.")
        else:
        ...
        # Uses the PID file (scoped to CAESAR_HOME) to find this
        # installation's gateway — safe with multiple installations.
        try:
            from gateway.status import get_running_pid, remove_pid_file
            from caesar_cli.gateway import (
            ...
            # Also check for a system-level service (caesar gateway install --system).
            ...
            if is_macos():
                try:
                    from caesar_cli.gateway import get_launchd_label
            ...
            if existing_pid or has_systemd_service or has_system_service or has_launchd_service:
                ...
                if has_systemd_service:
                    ...
                    if restart.returncode == 0:
                        ...
                        if is_linux():
                            ...
                            if linger_ok is not True:
                                ...
                                print("    caesar gateway restart")
                            else:
                                print("  Try manually: caesar gateway restart")
                elif has_system_service:
                    # System-level service (caesar gateway install --system).
                    ...
                    print("  Restart it with: caesar gateway run")
        except Exception as e:
            logger.debug("Gateway restart during update failed: %s", e)
        
        print()
        print("Tip: You can now select a provider and model:")
        print("  caesar model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``caesar -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from caesar_cli.profiles import (
    ...
    from caesar_constants import display_caesar_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `caesar profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_caesar_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → caesar -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.caesar)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  caesar profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     caesar -p {name} chat")
                else:
            ...
            if clone or clone_all:
                from caesar_constants import get_caesar_home
                profile_dir_display = f"~/.caesar/profiles/{name}"
        ...
        name = args.profile_name
        from caesar_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from caesar_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from caesar_cli.profiles import rename_profile
        try:
        ...
        from caesar_cli.profiles import export_profile
        name = args.profile_name
        ...
        from caesar_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from caesar_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def main():
    """Main entry point for caesar CLI."""
    parser = argparse.ArgumentParser(
        prog="caesar",
        description="Caesar Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    caesar                        Start interactive chat
    caesar chat -q "Hello"        Single query mode
    caesar -c                     Resume the most recent session
    caesar -c "my project"        Resume a session by name (latest in lineage)
    caesar --resume <session_id>  Resume a specific session by ID
    caesar setup                  Run setup wizard
    caesar logout                 Clear stored authentication
    caesar auth add <provider>    Add a pooled credential
    caesar auth list              List pooled credentials
    caesar auth remove <p> <n>    Remove pooled credential by index
    caesar auth reset <provider>  Clear exhaustion status for a provider
    caesar model                  Select default model
    caesar config                 View configuration
    caesar config edit            Edit config in $EDITOR
    caesar config set model gpt-4 Set a config value
    caesar gateway                Run messaging gateway
    caesar -s caesar-agent-dev,github-auth
    caesar -w                     Start in isolated git worktree
    caesar gateway install        Install gateway background service
    caesar sessions list          List past sessions
    caesar sessions browse        Interactive session picker
    caesar sessions rename ID T   Rename/title a session
    caesar update                 Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Caesar Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: caesar-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Caesar Agent with an interactive wizard. "
                    "Run a specific section: caesar setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Caesar CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: caesar-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Caesar Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Caesar Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Caesar Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from caesar_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from caesar_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from caesar_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/caesar-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from caesar_cli.plugins_cmd import plugins_command
    ...
    # Provider selection happens via 'caesar memory setup'.
    # =========================================================================
    honcho_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "plugin — initial setup is via 'caesar memory setup'. These commands\n"
    ...
    honcho_subparsers.add_parser("setup", help="Initial Honcho setup (redirects to caesar memory setup)")
    ...
    honcho_subparsers.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Caesar Honcho",
    )
    ...
    def cmd_honcho(args):
        ...
        if sub == "setup":
            ...
            print("  Running 'caesar memory setup'...\n")
            from caesar_cli.memory_setup import memory_command
            memory_command(args)
            return
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'caesar tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # caesar tools list [--platform cli]
    ...
    # caesar tools disable <name...> [--platform cli]
    ...
    # caesar tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from caesar_cli.tools_config import tools_disable_enable_command
            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from caesar_cli.tools_config import tools_command
            tools_command(args)
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Caesar as an MCP server",
        description=(
            "Manage MCP server connections and run Caesar as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'caesar mcp add' to connect to a new server, or\n"
            "'caesar mcp serve' to expose Caesar conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Caesar as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from caesar_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch caesar --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            caesar_bin = shutil.which("caesar")
            if caesar_bin:
                os.execvp(caesar_bin, ["caesar", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "caesar_cli.main", "--resume", selected_id],
                )
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Caesar"
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Caesar",
    ...
    def cmd_claw(args):
        from caesar_cli.claw import claw_command
        claw_command(args)
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Caesar Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Caesar Agent",
        description="Remove Caesar Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Caesar Agent as an ACP (Agent Client Protocol) server",
        description="Start Caesar Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Caesar Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated Caesar instances",
    )
    ...
    # e.g. ``caesar -c Pokemon Agent Dev`` → ``caesar -c 'Pokemon Agent Dev'``
