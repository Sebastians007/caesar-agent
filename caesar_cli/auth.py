"""
Multi-provider authentication system for Caesar Agent.

Supports OAuth device code flows (Nous Portal, future: OpenAI Codex) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.caesar/auth.json with cross-process file locking.
...
from caesar_cli.config import get_caesar_home, get_config_path
...
DEFAULT_NOUS_CLIENT_ID = "caesar-cli"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from caesar_cli.copilot_auth import resolve_copilot_token
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `caesar model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("CAESAR_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.caesar/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    return get_caesar_home() / "auth.json"
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `caesar logout`.
...
def resolve_provider(
    ...
    *,
    ...
    raise AuthError(
        "No inference provider configured. Run 'caesar model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.caesar/.env.",
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.caesar/auth.json (not ~/.codex/)
#
# Caesar maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Caesar auth store (~/.caesar/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `caesar login` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `caesar login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `caesar login` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `caesar login` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to Caesar auth store (~/.caesar/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating Caesar auth state."""
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to Caesar auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Caesar's own Codex token store."""
    try:
        ...
        if cli_tokens:
            logger.info("Migrating Codex credentials from ~/.codex/ to Caesar auth store")
            print("⚠️  Migrating Codex credentials to Caesar's own auth store.")
            print("   This avoids conflicts with Codex CLI and VS Code.")
            print("   Run `caesar login` to create a fully independent session.\n")
    ...
    refresh_timeout_seconds = float(os.getenv("CAESAR_CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Caesar processes
    ...
    base_url = (
        os.getenv("CAESAR_CODEX_BASE_URL", "").strip().rstrip("/")
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Caesar models — they're not reliable for agentic tool-calling
            if "caesar" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Caesar is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or os.getenv("CAESAR_PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "caesar-cli"),
...
def resolve_nous_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Caesar is not logged into Nous Portal.",
...
def get_nous_auth_status() -> Dict[str, Any]:
    """Status snapshot for `caesar status` output."""
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("CAESAR_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("CAESAR_COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set CAESAR_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def detect_external_credentials() -> List[Dict[str, Any]]:
    """Scan for credentials from other CLI tools that Caesar can reuse.

    Returns a list of dicts, each with:
      - provider: str   -- Caesar provider id (e.g. "openai-codex")
    ...
    if cli_tokens:
        ...
        found.append({
            "provider": "openai-codex",
            "path": str(codex_path),
            "label": f"Codex CLI credentials found ({codex_path}) — run `caesar login` to create a separate session",
        })
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from caesar_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'caesar model' or 'caesar setup' instead."""
    print("The 'caesar login' command has been removed.")
    print("Use 'caesar model' to select a provider and model,")
    print("or 'caesar setup' for full interactive setup.")
    raise SystemExit(0)


def _login_openai_codex(args, pconfig: ProviderConfig) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.caesar/auth.json."""

    # Check for existing Caesar-owned credentials
    try:
        existing = resolve_codex_runtime_credentials()
        print("Existing Codex credentials found in Caesar auth store.")
        try:
    ...
    if cli_tokens:
        ...
        print("Caesar will create its own session to avoid conflicts with Codex CLI / VS Code.")
        try:
        ...
        if do_import in ("y", "yes"):
            _save_codex_tokens(cli_tokens)
            base_url = os.getenv("CAESAR_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
            config_path = _update_config_for_provider("openai-codex", base_url)
            print()
            print("Credentials imported. Note: if Codex CLI refreshes its token,")
            print("Caesar will keep working independently with its own session.")
            print(f"  Config updated: {config_path} (model.provider=openai-codex)")
            return

    # Run a fresh device code flow — Caesar gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Caesar creates its own session — won't affect Codex CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Caesar auth store
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting Caesar login via {pconfig.name}...")
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target):
        ...
        if os.getenv("OPENROUTER_API_KEY"):
            print("Caesar will use OpenRouter for inference.")
        else:
            print("Run `caesar model` or configure an API key to use Caesar.")
    else:
