"""caesar claw — OpenClaw migration commands.

Usage:
    caesar claw migrate              # Interactive migration from ~/.openclaw
    caesar claw migrate --dry-run    # Preview what would be migrated
    caesar claw migrate --preset full --overwrite  # Full migration, overwrite conflicts
    caesar claw cleanup              # Archive leftover OpenClaw directories
    caesar claw cleanup --dry-run    # Preview what would be archived
"""
...
from caesar_cli.config import get_caesar_home, get_config_path, load_config, save_config
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_caesar.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_caesar", script_path)
...
def claw_command(args):
    ...
    if action == "migrate":
        ...
        print("Usage: caesar claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from OpenClaw to Caesar")
        print("  cleanup          Archive leftover OpenClaw directories after migration")
        print()
        print("Run 'caesar claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the OpenClaw → Caesar migration."""
    ...
    if not source_dir.is_dir():
        ...
        print_info("You can specify a custom path: caesar claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {caesar_home}")
...
def _offer_source_archival(source_dir: Path, auto_yes: bool = False):
    ...
    if auto_yes or prompt_yes_no(f"Archive {source_dir} now?", default=True):
        ...
        print_info("Skipped. You can archive later with: caesar claw cleanup")
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  caesar claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  caesar claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  caesar config set OPENROUTER_API_KEY sk-or-v1-...")
