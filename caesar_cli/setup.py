"""
Interactive setup wizard for Caesar Agent.
...
Config files are stored in ~/.caesar/ for easy access.
"""
...
from caesar_constants import get_optional_skills_dir
...
def _setup_copilot_reasoning_selection(
    ...
    *,
    ...
    from caesar_cli.models import github_model_reasoning_efforts, normalize_copilot_model_id
...
def _setup_provider_model_selection(config, provider_id, current_model, prompt_choice, prompt_fn):
    ...
    """
    from caesar_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    from caesar_cli.config import get_env_value
...
# display_caesar_home imported lazily at call sites (stale-module safety during caesar update)

from caesar_cli.colors import Colors, color
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Caesar Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Caesar using environment variables or config commands:")
    print_info("  caesar config set model.provider custom")
    print_info("  caesar config set model.base_url http://localhost:8080/v1")
    print_info("  caesar config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'caesar setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from caesar_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'caesar setup')")


def _print_setup_summary(config: dict, caesar_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'caesar setup' to configure"))
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'caesar setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'caesar setup terminal'"))
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'caesar setup tools' to configure them,"
        )
        from caesar_constants import display_caesar_home as _dhh
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {caesar_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('caesar setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('caesar setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('caesar setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('caesar setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('caesar setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('caesar config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('caesar config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('caesar config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('caesar', Colors.GREEN)}              Start chatting")
    print(f"   {color('caesar gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('caesar doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `caesar tools` and `caesar setup tools`).
...
def setup_model_provider(config: dict):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``caesar model``)
    ...
    provider added to ``caesar model`` is automatically available here.
    """
    ...
    # Delegate to the shared caesar model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from caesar_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: caesar model")
    ...
    if _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from caesar_cli.auth_commands import auth_add_command
            ...
            print_info(
                "Caesar can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if selected_provider in _vision_backends:
        # If the user just selected a backend Caesar can already use for
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one Caesar can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'caesar setup' or configure AUXILIARY_VISION_* settings")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.caesar/.env."
            )
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'caesar setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Caesar runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using Caesar via Telegram/Discord, this is where")
        print_info(
...
def setup_agent_settings(config: dict):
    ...
    current_max = get_env_value("CAESAR_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("CAESAR_MAX_ITERATIONS", str(max_iter))
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Caesar from anywhere.")
    print()
    ...
    if not existing_telegram and prompt_yes_no("Set up Telegram bot?", False):
        ...
        if token:
            ...
            print()
            print_info("📬 Home Channel: where Caesar delivers cron job results,")
    ...
    if not existing_slack and prompt_yes_no("Set up Slack bot?", False):
        ...
        print()
        print_info(
            "   Full guide: https://github.com/Sebastians007/caesar-agent/blob/main/website/docs/user-guide/messaging/slack.md"
        )
        print()
    ...
    if not existing_matrix and prompt_yes_no("Set up Matrix?", False):
        ...
        if token or get_env_value("MATRIX_PASSWORD"):
            ...
            # Home room
            print()
            print_info("📬 Home Room: where Caesar delivers cron job results and notifications.")
    ...
    if not existing_mattermost and prompt_yes_no("Set up Mattermost?", False):
        ...
        if token:
            ...
            # Home channel
            print()
            print_info("📬 Home Channel: where Caesar delivers cron job results and notifications.")
    ...
    if not existing_whatsapp and prompt_yes_no("Set up WhatsApp?", False):
        ...
        print_info("Requires Node.js. Run 'caesar whatsapp' for guided setup.")
        print()
        if prompt_yes_no("Enable WhatsApp now?", True):
            save_env_value("WHATSAPP_ENABLED", "true")
            print_success("WhatsApp enabled")
            print_info("Run 'caesar whatsapp' to choose your mode (separate bot number")
    ...
    if not existing_webhook and prompt_yes_no("Set up webhooks? (GitHub, GitLab, etc.)", False):
        ...
        print()
        print_info(
            "   Full guide: https://github.com/Sebastians007/caesar-agent/blob/main/website/docs/user-guide/messaging/webhooks.md"
        )
        print()
        ...
        print_info(
            "   https://github.com/Sebastians007/caesar-agent/blob/main/website/docs/user-guide/messaging/webhooks.md#configuring-routes"
        )
        print()
        print_info("   Open config in your editor:  caesar config edit")
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     caesar config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                try:
                    ...
                    print_info("  You can try manually: caesar gateway install")
            else:
                print_info("  You can install later: caesar gateway install")
                if _is_linux:
                    print_info("  Or as a boot-time service: sudo caesar gateway install --system")
                print_info("  Or run in foreground:  caesar gateway")
        else:
            print_info("Start the gateway to bring your bots online:")
            print_info("   caesar gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `caesar setup tools` and `caesar tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from caesar_cli.config
    ...
    if section_key == "model":
        ...
        if not has_key:
            ...
            try:
                from caesar_cli.auth import get_active_provider
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_caesar.py"
)


def _offer_openclaw_migration(caesar_home: Path) -> bool:
    ...
    print_info("Caesar can import your settings, memories, skills, and API keys.")
    print()
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "openclaw_to_caesar", _OPENCLAW_SCRIPT
        )
        ...
        migrator = mod.Migrator(
            ...
            target_root=caesar_home.resolve(),
            execute=True,
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Caesar.")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      caesar setup           — full or quick (auto-detected)
      caesar setup model     — just model/provider
      caesar setup terminal  — just terminal backend
      caesar setup gateway   — just messaging platforms
      caesar setup tools     — just tool configuration
      caesar setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Caesar Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ Caesar Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Caesar Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Caesar configured.")
        print()
        ...
        if choice == 0:
            # Quick setup
            _run_quick_setup(config, caesar_home)
            return
        elif choice == 1:
            ...
            print_info("Exiting. Run 'caesar setup' again when ready.")
            return
        elif choice == 9:
            print_info("Exiting. Run 'caesar setup' again when ready.")
            return
            ...
            if section:
                ...
                func(config)
                save_config(config)
                _print_setup_summary(config, caesar_home)
            return
    else:
        # ── First-Time Setup ──
        print()
        ...
        migration_ran = _offer_openclaw_migration(caesar_home)
        if migration_ran:
    ...
    print_info(f"Data folder:  {caesar_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'caesar config edit'")
...
def _run_quick_setup(config: dict, caesar_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'caesar setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Caesar to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'caesar setup gateway'.")
