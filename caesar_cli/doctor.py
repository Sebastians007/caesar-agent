"""
Doctor command for caesar CLI.

Diagnoses issues with Caesar Agent setup.
"""
...
from caesar_cli.config import get_project_root, get_caesar_home, get_env_path
from caesar_constants import display_caesar_home

PROJECT_ROOT = get_project_root()
CAESAR_HOME = get_caesar_home()
_DHH = display_caesar_home()  # user-facing display path (e.g. ~/.caesar or ~/.caesar/profiles/coder)

# Load environment variables from ~/.caesar/.env so API key checks work
...
from caesar_constants import OPENROUTER_MODELS_URL
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.caesar/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `caesar`.
    os.environ.setdefault("CAESAR_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 Caesar Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.caesar/.env (primary location for user config)
    env_path = CAESAR_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'caesar setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'caesar setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'caesar setup' to create one")
                issues.append("Run 'caesar setup' to create .env")
    
    # Check ~/.caesar/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    try:
        from caesar_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    caesar_home = CAESAR_HOME
    if caesar_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            caesar_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = caesar_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Caesar a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Caesar Agent Persona\n\n"
                "<!-- Edit this file to customize how Caesar communicates. -->\n\n"
                "You are Caesar, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = caesar_home / "memories"
    ...
    state_db_path = caesar_home / "state.db"
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint — https://github.com/Sebastians007/caesar-agent/issues/811
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'caesar setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    hub_dir = CAESAR_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: caesar skills list)")
    ...
    try:
        ...
        if not _honcho_cfg_path.exists():
            check_warn("Honcho config not found", "run: caesar memory setup")
        elif not hcfg.enabled:
            check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
        elif not (hcfg.api_key or hcfg.base_url):
            check_fail("Honcho API key or base URL not set", "run: caesar memory setup")
            issues.append("No Honcho API key — run 'caesar memory setup'")
        else:
    ...
    # =========================================================================
    try:
        from caesar_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "caesar -p" in content:
                            _m = _re.search(r"caesar -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'caesar doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
