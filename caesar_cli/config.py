"""
Configuration management for Caesar Agent.

Config files are stored in ~/.caesar/ for easy access:
- ~/.caesar/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.caesar/.env         - API keys and secrets

This module provides:
- caesar config          - Show current configuration
- caesar config edit     - Open config in editor
- caesar config set      - Set a specific value
- caesar config wizard   - Re-run setup wizard
"""
...
from caesar_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("CAESAR_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_caesar_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Caesar is running in package-manager-managed mode.

    Two signals: the CAESAR_MANAGED env var (set by the systemd service),
    or a .managed marker file in CAESAR_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade caesar-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "caesar update"


def format_managed_message(action: str = "modify this Caesar installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("CAESAR_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Caesar installation is managed by NixOS "
            f"(CAESAR_MANAGED={env_hint}).\n"
            "Edit services.caesar-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Caesar installation is managed by Homebrew "
            f"(CAESAR_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade caesar-agent"
        )

    return (
        f"Cannot {action}: this Caesar installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Caesar."
    )
...
# Re-export from caesar_constants — canonical definition lives there.
from caesar_constants import get_caesar_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_caesar_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_caesar_home() / ".env"
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into CAESAR_HOME if the user doesn't have one yet."""
...
def ensure_caesar_home():
    """Ensure ~/.caesar directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Caesar sends a stable profile-scoped userId to Camofox
    ...
    # always goes to ~/.caesar/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for caesar-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Caesar Agent*" header.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Caesar auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @caesar:example.org)",
    ...
    # CAESAR_TOOL_PROGRESS and CAESAR_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "CAESAR_TOOL_PROGRESS": {
    ...
    "CAESAR_EPHEMERAL_SYSTEM_PROMPT": {
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("CAESAR_TOOL_PROGRESS")
            old_mode = get_env_value("CAESAR_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from CAESAR_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from CAESAR_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("CAESAR_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from CAESAR_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: caesar config set <key> <value>")
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.caesar/config.yaml."""
    import copy
    ensure_caesar_home()
...
#   openai-codex (OAuth — caesar login) — OpenAI Codex
#   nous         (OAuth — caesar login) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.caesar/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.caesar/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Caesar env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.caesar/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.caesar/.env."""
    if is_managed():
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.caesar/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ Caesar Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  caesar config edit     # Edit config file", Colors.DIM))
    print(color("  caesar config set <key> <value>", Colors.DIM))
    print(color("  caesar setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: caesar config set <key> <value>")
            print()
            print("Examples:")
            print("  caesar config set model anthropic/claude-sonnet-4")
            print("  caesar config set terminal.backend docker")
            print("  caesar config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'caesar config migrate' to add them")
        ...
        print("  caesar config           Show current configuration")
        print("  caesar config edit      Open config in editor")
        print("  caesar config set <key> <value>   Set a config value")
        print("  caesar config check     Check for missing/outdated config")
        print("  caesar config migrate   Update config with new options")
        print("  caesar config path      Show config file path")
        print("  caesar config env-path  Show .env file path")
        sys.exit(1)
