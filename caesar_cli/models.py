...
Add, remove, or reorder entries here — both `caesar setup` and
`caesar` provider-selection will pick up the change automatically.
"""
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from caesar_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Caesar' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``caesar_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from caesar_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from caesar_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from caesar_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "CaesarAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from caesar_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Caesar will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
