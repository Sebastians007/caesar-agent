...
Comparison of Caesar Agent vs. openclaw-honcho — and a porting spec for bringing Caesar patterns into other Honcho integrations.

---

## Overview

Two independent Honcho integrations have been built for two different agent runtimes: **Caesar Agent** (Python, baked into the runner) and **openclaw-honcho** (TypeScript plugin via hook/tool API). Both use the same Honcho peer paradigm — dual peer model, `session.context()`, `peer.chat()` — but they made different tradeoffs at every layer.

This document maps those tradeoffs and defines a porting spec: a set of Caesar-originated patterns, each stated as an integration-agnostic interface, that any Honcho integration can adopt regardless of runtime or language.

> **Scope** Both integrations work correctly today. This spec is about the delta — patterns in Caesar that are worth propagating and patterns in openclaw-honcho that Caesar should eventually adopt. The spec is additive, not prescriptive.

---

## Architecture comparison

### Caesar: baked-in runner
...
| Dimension | Caesar Agent | openclaw-honcho |
|---|---|---|
...
| **AI peer name in identity** | Replaces "Caesar Agent" in DEFAULT_AGENT_IDENTITY when configured. | Not implemented. |
...
Six patterns from Caesar are worth adopting in any Honcho integration. Each is described as an integration-agnostic interface.
...
**openclaw-honcho contributes back (Caesar should adopt):**
...
- [ ] **honcho mode / sessions / map** — CLI parity with Caesar
...
Greenfield integration. Start from openclaw-honcho's architecture and apply all Caesar patterns from day one.
