...
Caesar should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current Caesar behavior has four structural issues:
...
Caesar must stop keying pricing solely by `model`.
...
- manual `caesar pricing sync`

## Reconciliation Architecture

Live requests may produce only an estimate initially. Caesar should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. Caesar stores canonical usage plus reconciliation ids.
3. Caesar computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. Caesar should instead migrate to:
...
This is the minimum architecture that makes the statement "Caesar pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
