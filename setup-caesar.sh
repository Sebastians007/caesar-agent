...
# 5. Symlinks the 'caesar' CLI command into ~/.local/bin
...
echo ""
echo -e "${CYAN}⚕ Caesar Agent Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink caesar into ~/.local/bin
# ============================================================================

echo -e "${CYAN}→${NC} Setting up caesar command..."

CAESAR_BIN="$SCRIPT_DIR/venv/bin/caesar"
mkdir -p "$HOME/.local/bin"
ln -sf "$CAESAR_BIN" "$HOME/.local/bin/caesar"
echo -e "${GREEN}✓${NC} Symlinked caesar → ~/.local/bin/caesar"
...
# ============================================================================
# Seed bundled skills into ~/.caesar/skills/
# ============================================================================

CAESAR_SKILLS_DIR="${CAESAR_HOME:-$HOME/.caesar}/skills"
mkdir -p "$CAESAR_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.caesar/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$CAESAR_SKILLS_DIR/" 2>/dev/null || true
...
echo ""
echo "Other commands:"
echo "  caesar status        # Check configuration"
echo "  caesar gateway install # Install gateway service (messaging + cron)"
echo "  caesar cron list     # View scheduled jobs"
echo "  caesar doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m caesar_cli.main setup
fi
