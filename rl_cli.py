...
if tinker_atropos_dir.exists():
    ...
    os.environ['CAESAR_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to caesar-agent directory if submodule not found
...
from caesar_constants import get_caesar_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_caesar_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.caesar/config.yaml if not provided)
    ...
    """
    # Load config from ~/.caesar/config.yaml
    config = load_caesar_config()
