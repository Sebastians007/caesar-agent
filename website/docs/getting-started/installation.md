...
sidebar_position: 2
title: "Installation"
description: "Install Caesar Agent on Linux, macOS, or WSL2"
---

# Installation

Get Caesar Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Caesar Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `caesar` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
caesar gateway setup  # Set up messaging platforms
caesar config set     # Set individual config values
caesar setup          # Or run the full setup wizard to configure everything at once
```
...
:::tip
You do **not** need to activate the venv to use `caesar`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.caesar/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}
...
Open `~/.caesar/.env` and add at minimum an LLM provider key:
...
```bash
caesar config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `caesar` to Your PATH
...
```bash
caesar model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
caesar version    # Check that the command is available
caesar doctor     # Run diagnostics to verify everything is working
caesar status     # Check your configuration
caesar chat -q "Hello! What tools do you have available?"
```
...
# Make caesar available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/caesar" ~/.local/bin/caesar
...
| Problem | Solution |
|---------|----------|
| `caesar: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `caesar model` to configure your provider, or `caesar config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `caesar config check` then `caesar config migrate` |

For more diagnostics, run `caesar doctor` — it will tell you exactly what's missing and how to fix it.
