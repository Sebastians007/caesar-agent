...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Caesar Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Caesar Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.
...
caesar chat --model "anthropic/claude-sonnet-4"

# With a specific provider
caesar chat --provider nous        # Use Nous Portal
caesar chat --provider openrouter  # Force OpenRouter
...
caesar chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
caesar --continue             # Resume the most recent CLI session (-c)
caesar --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
caesar chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
caesar -w                         # Interactive mode in worktree
caesar -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the Caesar CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Caesar CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Caesar to background (Unix only). Run `fg` in the shell to resume. |
| `Tab` | Accept auto-suggestion (ghost text) or autocomplete slash commands |

## Slash Commands

Type `/` to see the autocomplete dropdown. Caesar supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Caesar replies |
...
```bash
caesar -s caesar-agent-dev,github-auth
caesar chat -s github-pr-workflow -s github-auth
```

Caesar loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.caesar/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.caesar/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Caesar..."
  # Add your own!
```
...
On Unix systems, press **`Ctrl+Z`** to suspend Caesar to the background — just like any terminal process. The shell prints a confirmation:

```
Caesar Agent has been suspended. Run `fg` to bring Caesar Agent back.
```
...
Resume this session with:
  caesar --resume 20260225_143052_a1b2c3
...
```bash
caesar --continue                          # Resume the most recent CLI session
caesar -c                                  # Short form
caesar -c "my project"                     # Resume a named session (latest in lineage)
caesar --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
caesar --resume "refactoring auth"         # Resume by title
caesar -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `caesar sessions rename <id> <title>` from the command line. Use `caesar sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Caesar's SQLite state database under `~/.caesar/state.db`. The database keeps:
...
Caesar immediately confirms the task and gives you back the prompt:
