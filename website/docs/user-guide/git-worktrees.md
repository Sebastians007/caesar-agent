...
title: "Git Worktrees"
description: "Run multiple Caesar agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Caesar Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Caesar so each session has a clean, isolated working directory.

## Why Use Worktrees with Caesar?

Caesar treats the **current working directory** as the project root:

- CLI: the directory where you run `caesar` or `caesar chat`
...
git worktree add ../repo-feature feature/caesar-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/caesar-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Caesar there:

```bash
cd ../repo-feature

# Start Caesar in the worktree
caesar
```
...
git worktree add ../repo-experiment-a feature/caesar-a
git worktree add ../repo-experiment-b feature/caesar-b
```
...
- Works on its own branch (`feature/caesar-a` vs `feature/caesar-b`).
...
- Caesar checkpoint data under `~/.caesar/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Caesar experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/caesar-checkpoints-docs`, `feature/caesar-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Caesar from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `caesar -w` (Automatic Worktree Mode)

Caesar has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `caesar/caesar-<hash>`).
...
```bash
caesar -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `caesar -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Caesar session its own clean checkout.
