...
Caesar Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Caesar checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.caesar/config.yaml`:
...
1. **CLI flag**: Start a session with `caesar --yolo` or `caesar chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `CAESAR_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `CAESAR_YOLO_MODE` environment variable which is checked before every command execution.
...
Configure the timeout in `~/.caesar/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.caesar/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.caesar/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` caesar/gateway | Self-termination prevention |
...
The `CAESAR_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.caesar/config.yaml`:
...
:::tip
Use `caesar config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Caesar controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.caesar/.env`:
...
For more flexible authorization, Caesar includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `caesar pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.caesar/config.yaml`:
...
caesar pairing approve telegram ABC12DEF

# Revoke a user's access
caesar pairing revoke telegram 123456789

# Clear all pending codes
caesar pairing clear-pending
```

**Storage:** Pairing data is stored in `~/.caesar/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Caesar applies strict security hardening to every container.
...
Container resources are configurable in `~/.caesar/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.caesar/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` in `~/.caesar/`. Skills declare these in frontmatter:
...
When loaded, Caesar checks if these files exist in `~/.caesar/` and registers them for mounting:
...
Paths are relative to `~/.caesar/`. Files are mounted to `/root/.caesar/` inside the container.
...
| **terminal** (local) | Blocks explicit Caesar infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Caesar infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Caesar integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
4. **Store secrets securely** — keep API keys in `~/.caesar/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.caesar/logs/` for unauthorized access attempts
10. **Keep updated** — run `caesar update` regularly for security patches

### Securing API Keys

```bash
# Set proper permissions on the .env file
chmod 600 ~/.caesar/.env
