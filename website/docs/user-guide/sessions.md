...
Caesar Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.caesar/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.caesar/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`caesar` or `caesar chat`) |
...
caesar -c "my project"   # → resumes "my project #3"
```
...
caesar --resume 20250305_091523_a1b2c3d4
caesar -r 20250305_091523_a1b2c3d4

# Resume by title
caesar --resume "refactoring auth"

# Or with the chat subcommand
caesar chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `caesar sessions list`.

### Conversation Recap on Resume

When you resume a session, Caesar displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Caesar session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.caesar/config.yaml`:
...
Caesar automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `caesar sessions list` or `caesar sessions browse`.
...
```bash
caesar sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Caesar creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`caesar -c "my project"`), it automatically picks the most recent session in the lineage.
...
Caesar provides a full set of session management commands via `caesar sessions`:
...
# Filter by platform
caesar sessions list --source telegram

# Show more sessions
caesar sessions list --limit 50
```
...
caesar sessions export backup.jsonl

# Export sessions from a specific platform
caesar sessions export telegram-history.jsonl --source telegram

# Export a single session
caesar sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
caesar sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
caesar sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
caesar sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
caesar sessions prune

# Custom age threshold
caesar sessions prune --older-than 30

# Only prune sessions from a specific platform
caesar sessions prune --source telegram --older-than 60

# Skip confirmation
caesar sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`caesar insights`](/docs/reference/cli-commands#caesar-insights).
...
When Caesar cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Caesar uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Caesar in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.caesar/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.caesar/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.caesar/sessions/sessions.json` | Maps session keys to active session IDs |
...
caesar sessions delete <session_id>
