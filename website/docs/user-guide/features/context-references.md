...
Type `@` followed by a reference to inject content directly into your message. Caesar expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Caesar env: `$CAESAR_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$CAESAR_HOME/skills/.hub/`
