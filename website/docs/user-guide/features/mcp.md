...
description: "Connect Caesar Agent to external tool servers via MCP — and control exactly which MCP tools Caesar loads"
---

# MCP (Model Context Protocol)

MCP lets Caesar Agent connect to external tool servers so the agent can use tools that live outside Caesar itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Caesar to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Caesar tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Caesar to see
...
```bash
cd ~/.caesar/caesar-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.caesar/config.yaml`:
...
4. Ask Caesar to use the MCP-backed capability.
...
Caesar will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Caesar connects to directly.
...
- you do not want Caesar spawning a local subprocess for that integration

## Basic configuration reference

Caesar reads MCP config from `~/.caesar/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Caesar skips the server entirely |
...
## How Caesar registers MCP tools

Caesar prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Caesar sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Caesar also registers utility tools around MCP resources and prompts:
...
- Caesar only registers resource utilities if the MCP session actually supports resource operations
- Caesar only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Caesar, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Caesar skips the server completely and does not even attempt a connection.
...
You can also separately disable Caesar-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Caesar does not create an empty runtime MCP toolset for that server.
...
Caesar discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Caesar when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Caesar receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Caesar does not blindly pass your full shell environment.
...
Then verify your config and restart Caesar.
...
Because Caesar now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Caesar via the `sampling/createMessage` protocol. This allows an MCP server to ask Caesar to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Caesar as an MCP server

In addition to connecting **to** MCP servers, Caesar can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Caesar's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Caesar
- You want a single MCP server that bridges to all of Caesar's connected messaging platforms at once
- You already have a running Caesar gateway with connected platforms
...
Add Caesar to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Caesar in a specific location:

```json
{
  "mcpServers": {
    "caesar": {
      "command": "/home/user/.caesar/caesar-agent/venv/bin/caesar",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Caesar-specific channel browser:
...
The MCP server includes a live event bridge that polls Caesar's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
caesar mcp serve              # Normal mode
caesar mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Caesar's session store (`~/.caesar/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Caesar agent itself.
