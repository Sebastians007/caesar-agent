...
description: "Overview of Caesar Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Caesar ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, Honcho memory, and more.
...
caesar chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `caesar-cli`, `caesar-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
caesar config set terminal.backend singularity
caesar config set terminal.singularity_image ~/python.sif
```
...
modal setup
caesar config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.caesar/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.caesar/.env`.
:::
