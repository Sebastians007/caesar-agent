...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Caesar Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Caesar Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Caesar](/docs/guides/use-voice-mode-with-caesar).

## Prerequisites

Before using voice features, make sure you have:

1. **Caesar Agent installed** — `pip install caesar-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `caesar model` or set your preferred provider credentials in `~/.caesar/.env`
3. **A working base setup** — run `caesar` to verify the agent responds to text before enabling voice

:::tip
The `~/.caesar/` directory and default `config.yaml` are created automatically the first time you run `caesar`. You only need to create `~/.caesar/.env` manually for API keys.
:::
...
pip install "caesar-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "caesar-agent[all]"
```
...
```bash
caesar                # Start the interactive CLI
```
...
1. Start the CLI with `caesar` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.caesar/config.yaml` (default: `ctrl+b`).
:::
...
```bash
caesar gateway        # Start the gateway (connects to configured platforms)
caesar gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@caesarbyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.caesar/.env`:
```bash
...
```bash
caesar gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.caesar/.env`
...
- Review gateway logs: `tail -f ~/.caesar/logs/gateway.log`
