...
sidebar_position: 14
title: "API Server"
description: "Expose caesar-agent as an OpenAI-compatible API for any frontend"
---

# API Server

The API server exposes caesar-agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to caesar-agent and use it as a backend.
...
# Optional: only if a browser must call Caesar directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "caesar-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Lists `caesar-agent` as an available model. Required by most frontends for model discovery.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), caesar-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call Caesar directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.

:::warning Security
The API server gives full access to caesar-agent's toolset, **including terminal commands**. If you change the bind address to `0.0.0.0` (network-accessible), **always set `API_SERVER_KEY`** and keep `API_SERVER_CORS_ORIGINS` narrow — without that, remote callers may be able to execute arbitrary commands on your machine.
