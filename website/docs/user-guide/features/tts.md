...
Caesar Agent supports both text-to-speech output and voice message transcription across all messaging platforms.
...
| CLI | Saved to `~/.caesar/audio_cache/` | MP3 |
...
:::info Zero Config
Local transcription works out of the box when `faster-whisper` is installed. If that's unavailable, Caesar can also use a local `whisper` CLI from common install locations (like `/opt/homebrew/bin`) or a custom command via `CAESAR_LOCAL_STT_COMMAND`.
:::
...
**Custom local CLI fallback** — Set `CAESAR_LOCAL_STT_COMMAND` if you want Caesar to call a local transcription command directly. The command template supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders.

### Fallback Behavior

If your configured provider isn't available, Caesar automatically falls back:
- **Local faster-whisper unavailable** → Tries a local `whisper` CLI or `CAESAR_LOCAL_STT_COMMAND` before cloud providers
