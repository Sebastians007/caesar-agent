...
sidebar_position: 8
title: "Context Files"
description: "Project context files — .caesar.md, AGENTS.md, CLAUDE.md, global SOUL.md, and .cursorrules — automatically injected into every conversation"
---

# Context Files

Caesar Agent automatically discovers and loads context files that shape how it behaves. Some are project-local and discovered from your working directory. `SOUL.md` is now global to the Caesar instance and is loaded from `CAESAR_HOME` only.

## Supported Context Files

| File | Purpose | Discovery |
|------|---------|-----------| 
| **.caesar.md** / **CAESAR.md** | Project instructions (highest priority) | Walks to git root |
...
| **SOUL.md** | Global personality and tone customization for this Caesar instance | `CAESAR_HOME/SOUL.md` only |
| **.cursorrules** | Cursor IDE coding conventions | CWD only |
| **.cursor/rules/*.mdc** | Cursor IDE rule modules | CWD only |

:::info Priority system
Only **one** project context type is loaded per session (first match wins): `.caesar.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. **SOUL.md** is always loaded independently as the agent identity (slot #1).
:::
...
Caesar walks the directory tree starting from the working directory and loads **all** `AGENTS.md` files found, sorted by depth. This supports monorepo-style setups:
...
- `~/.caesar/SOUL.md`
- or `$CAESAR_HOME/SOUL.md` if you run Caesar with a custom home directory

Important details:

- Caesar seeds a default `SOUL.md` automatically if one does not exist yet
- Caesar loads `SOUL.md` only from `CAESAR_HOME`
- Caesar does not probe the working directory for `SOUL.md`
...
Caesar is compatible with Cursor IDE's `.cursorrules` file and `.cursor/rules/*.mdc` rule modules. If these files exist in your project root and no higher-priority context file (`.caesar.md`, `AGENTS.md`, or `CLAUDE.md`) is found, they're loaded as the project context.

This means your existing Cursor conventions automatically apply when using Caesar.
