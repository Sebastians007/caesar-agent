...
description: "Customize Caesar Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Caesar Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `CAESAR_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Caesar is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Caesar now seeds a default `SOUL.md` automatically in:

```text
~/.caesar/SOUL.md
```

More precisely, it uses the current instance's `CAESAR_HOME`, so if you run Caesar with a custom home directory, it will use:
...
- Caesar creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Caesar loads `SOUL.md` only from `CAESAR_HOME`
- Caesar does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Caesar falls back to a built-in default identity
...
If Caesar loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `CAESAR_HOME`, the personality belongs to the Caesar instance itself.

That also makes it easier to teach users:
- "Edit `~/.caesar/SOUL.md` to change Caesar' default personality."
...
- how Caesar should handle uncertainty, disagreement, or ambiguity
...
## What Caesar injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Caesar falls back to a built-in default identity ("You are Caesar Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Caesar ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Caesar, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Caesar its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.caesar/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.caesar/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-caesar)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Caesar speaks
- `display.skin` and `/skin` affect how Caesar looks in the terminal
