...
Caesar has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `caesar_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.caesar/plans/` relative to the active workspace/backend working directory.
...
| `/plan [request]` | Load the bundled `plan` skill to write a markdown plan instead of executing the work. Plans are saved under `.caesar/plans/` relative to the active workspace/backend working directory. |
...
User-defined quick commands from `quick_commands` in `~/.caesar/config.yaml` are also available as slash commands. These are resolved at dispatch time, not shown in the built-in autocomplete/help tables.
