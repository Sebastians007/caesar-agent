...
description: "Frequently asked questions and solutions to common issues with Caesar Agent"
---
...
### What LLM providers work with Caesar?

Caesar Agent works with any OpenAI-compatible API. Supported providers include:
...
Set your provider with `caesar model` or by editing `~/.caesar/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Caesar Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Caesar from inside it. The standard install command works perfectly in WSL2:
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Caesar Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.caesar/`.

### Can I use it offline / with local models?

Yes. Run `caesar model`, select **Custom endpoint**, and enter your server's URL:
...
Caesar persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Caesar — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

### How much does it cost?

Caesar Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Caesar Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Caesar programmatically:

```python
from caesar.agent import AIAgent

agent = AIAgent(model="openrouter/nous/caesar-3-llama-3.1-70b")
...
#### `caesar: command not found` after installation
...
**Cause:** Caesar requires Python 3.11 or newer.
...
# Or set directly
caesar config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.caesar/.env` for conflicting entries.
:::
...
# Set a valid model
caesar config set CAESAR_MODEL openrouter/nous/caesar-3-llama-3.1-70b

# Or specify per-session
caesar chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `caesar chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Caesar detected the wrong context length for your model.
...
caesar chat --model openrouter/google/gemini-2.0-flash-001
```

If this happens on the first long conversation, Caesar may have the wrong context length for your model. Check what it detected:
...
**Cause:** Caesar detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Caesar never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `caesar chat`
...
# Start the gateway
caesar gateway start

# Check logs for errors
cat ~/.caesar/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `caesar gateway setup`
- Check gateway logs: `cat ~/.caesar/logs/gateway.log | tail -50`
...
Configure in `~/.caesar/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "caesar-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
**Solution:** The gateway captures your shell PATH when you run `caesar gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
caesar gateway install    # Re-snapshots your current PATH
caesar gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.caesar.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `caesar chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `caesar chat -t "terminal"`
...
cd ~/.caesar/caesar-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.caesar/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
caesar config show | grep -A 12 mcp_servers

# Restart Caesar or reload MCP after config changes
caesar chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Caesar](/docs/guides/use-mcp-with-caesar)
...
:::warning
If an MCP server crashes mid-request, Caesar will report a timeout. Check the server's own logs (not just Caesar logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `caesar profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `caesar update`?

`caesar update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `caesar update` once — it covers every profile on the machine.
...
caesar profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
caesar profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.caesar/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
1. **Search existing issues:** [GitHub Issues](https://github.com/Sebastians007/caesar-agent/issues)
2. **Ask the community:** [GitHub Issues](https://github.com/Sebastians007/caesar-agent/issues)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Caesar version (`caesar --version`), and the full error message
