...
description: "How Caesar stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---

# Cron Internals

Caesar cron support is implemented primarily in:
...
Cron jobs are stored in Caesar-managed local state (`~/.caesar/cron/jobs.json`) with atomic write semantics.
...
A cron job may attach multiple skills. At runtime, Caesar loads those skills in order and then appends the job prompt as the task instruction.
...
Caesar uses lock-based protections so overlapping scheduler ticks do not execute the same due-job batch twice.
