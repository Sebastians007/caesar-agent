...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Caesar-Agent Atropos integration"
---
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Caesar/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- CaesarAgentBaseEnv
    CaesarAgentBaseEnv <|-- TerminalTestEnv
    CaesarAgentBaseEnv <|-- CaesarSweEnv
    CaesarAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The caesar-agent layer (`environments/caesar_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls caesar-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `CaesarAgentLoop` and scores the result
...
Your environment inherits from `CaesarAgentBaseEnv` and implements five methods:
...
`CaesarAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as caesar-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any caesar-agent tool |
...
parser = get_parser("caesar")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `caesar`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
pip install "caesar-agent[yc-bench]"
...
```bash
python environments/caesar_swe_env/caesar_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"caesar"`, `"qwen"`, `"mistral"`)
...
```python
from environments.caesar_base_env import CaesarAgentBaseEnv, CaesarAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(CaesarAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(CaesarAgentBaseEnv):
    name = "my-env"
...
### CaesarAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which caesar toolsets to enable |
...
| `tool_call_parser` | `str` | `"caesar"` | Parser for Phase 2 raw output |
...
- [Modal](https://modal.com) account and CLI: `pip install "caesar-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "caesar-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── caesar_base_env.py          # Abstract base class (CaesarAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (CaesarAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── caesar_parser.py        # Caesar/ChatML <tool_call> format
...
├── caesar_swe_env/             # SWE-bench training environment
│
