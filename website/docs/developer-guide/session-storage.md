...
Caesar Agent uses a SQLite database (`~/.caesar/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source file: `caesar_state.py`


## Architecture Overview

```
~/.caesar/state.db (SQLite, WAL mode)
...
Multiple caesar processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.caesar/state.db
...
Default path: `~/.caesar/state.db`

This is derived from `caesar_constants.get_caesar_home()` which resolves to
`~/.caesar/` by default, or the value of `CAESAR_HOME` environment variable.
