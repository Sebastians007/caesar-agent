...
Caesar currently supports three API execution modes:
...
Caesar wraps API requests so they can be interrupted from the CLI or gateway.
...
Caesar uses two execution strategies:
...
Caesar tracks a shared iteration budget across parent and subagents. It also injects budget pressure hints near the end of the available iteration window.
...
Before and during long runs, Caesar may:
