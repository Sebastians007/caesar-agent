...
sidebar_position: 1
title: "Architecture"
description: "Caesar Agent internals — major subsystems, execution paths, and where to read next"
---

# Architecture

This page is the top-level map of Caesar Agent internals. The project has grown beyond a single monolithic loop, so the best way to understand it is by subsystem.
...
├── caesar_state.py           # SQLite session/state database
├── batch_runner.py           # batch trajectory generation
│
├── agent/                    # prompt building, compression, caching, metadata, trajectories
├── caesar_cli/               # command entrypoints, auth, setup, models, config, doctor
...
├── environments/             # Caesar RL / benchmark environment framework
...
Caesar has a shared runtime provider resolver used by CLI, gateway, cron, ACP, and auxiliary calls.
...
ACP exposes Caesar as an editor-native agent over stdio/JSON-RPC.
...
Caesar ships a full environment framework for evaluation, RL integration, and SFT data generation.
...
The older mental model of Caesar as “one OpenAI-compatible chat loop plus some tools” is no longer sufficient. Current Caesar includes:
