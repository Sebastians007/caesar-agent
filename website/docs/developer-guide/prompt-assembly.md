...
description: "How Caesar builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Caesar deliberately separates:
...
1. agent identity — `SOUL.md` from `CAESAR_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.caesar/SOUL.md)
You are Caesar, an AI assistant created by Sebastian Sartele.
...
`SOUL.md` lives at `~/.caesar/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_caesar_md(cwd_path)       # 1. .caesar.md / CAESAR.md (walks to git root)
    ...
    # SOUL.md from CAESAR_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.caesar.md`, `CAESAR.md` | CWD up to git root | Caesar-native project config |
...
- **YAML frontmatter stripped** — `.caesar.md` frontmatter is removed (reserved for future config overrides)
...
1. `.caesar.md` / `CAESAR.md` (walks to git root)
