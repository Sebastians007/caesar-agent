---
sidebar_position: 6
title: "Use SOUL.md with Caesar"
description: "How to use SOUL.md to shape Caesar Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Caesar

`SOUL.md` is the **primary identity** for your Caesar instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Caesar to feel like the same assistant every time you talk to it — or if you want to replace the Caesar persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Caesar should be
- what Caesar should avoid stylistically
- how Caesar should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Caesar is and how Caesar speaks
...
Caesar now uses only the global SOUL file for the current instance:

```text
~/.caesar/SOUL.md
```

If you run Caesar with a custom home directory, it becomes:
...
Caesar automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Caesar does not overwrite it
- if the file exists but is empty, Caesar adds nothing from it to the prompt

## How Caesar uses it

When Caesar starts a session, it reads `SOUL.md` from `CAESAR_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Caesar feels.
...
Caesar already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Caesar should sound.

# Avoid
What Caesar should not do.

# Defaults
How Caesar should behave when ambiguity appears.
```
...
Then restart Caesar or start a new session.
...
### I edited SOUL.md but Caesar still sounds the same

Check:
- you edited `~/.caesar/SOUL.md` or `$CAESAR_HOME/SOUL.md`
...
### Caesar is ignoring parts of my SOUL.md
