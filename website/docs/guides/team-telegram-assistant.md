...
This tutorial walks you through setting up a Telegram bot powered by Caesar Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Caesar Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/learning-path) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.caesar/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Caesar itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Caesar Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_caesar_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Caesar Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Caesar Gateway...
...
# View live logs
journalctl --user -u caesar-gateway -f
...
caesar gateway start
caesar gateway stop
tail -f ~/.caesar/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `caesar gateway install` to update the plist.
:::
...
```bash
caesar gateway stop && caesar gateway start
```
...
caesar pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.caesar/.env`:
...
Control how much detail the bot shows when using tools. In `~/.caesar/config.yaml`:
...
Customize how the bot communicates by editing `~/.caesar/SOUL.md`:

For a full guide, see [Use SOUL.md with Caesar](/docs/guides/use-soul-with-caesar).
...
```bash
# From the CLI
caesar cron list          # View all scheduled jobs
caesar cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u caesar-gateway` (Linux) or `~/.caesar/logs/gateway.log` (macOS) |
| Cron job output | `~/.caesar/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.caesar/cron/jobs.json` |
| Pairing data | `~/.caesar/pairing/` |
| Session history | `~/.caesar/sessions/` |
