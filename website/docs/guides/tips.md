...
description: "Practical advice to get the most out of Caesar Agent — prompt tips, CLI shortcuts, context files, memory, cost optimization, and security"
---

# Tips & Best Practices

A quick-wins collection of practical tips that make you immediately more effective with Caesar Agent. Each section targets a different aspect — scan the headers and jump to what's relevant.
...
Forgot something from your last session? Run `caesar -c` to resume exactly where you left off, with full conversation history restored. You can also resume by title: `caesar -r "my research project"`.
...
Want Caesar to have a stable default voice? Edit `~/.caesar/SOUL.md` (or `$CAESAR_HOME/SOUL.md` if you use a custom Caesar home). Caesar now seeds a starter SOUL automatically and uses that global file as the instance-wide personality source.

For a full walkthrough, see [Use SOUL.md with Caesar](/docs/guides/use-soul-with-caesar).
...
Already have a `.cursorrules` or `.cursor/rules/*.mdc` file? Caesar reads those too. No need to duplicate your coding conventions — they're loaded automatically from the working directory.

### Hierarchical Discovery

Caesar walks the directory tree and discovers **all** `AGENTS.md` files at every level. In a monorepo, put project-wide conventions at the root and team-specific ones in subdirectories — they're all concatenated together with path headers.
...
Name your sessions with `/title auth-refactor` or `/title research-llm-quantization`. Named sessions are easy to find with `caesar sessions list` and resume with `caesar -r "auth-refactor"`. Unnamed sessions pile up and become impossible to distinguish.

### DM Pairing for Team Access

Instead of manually collecting user IDs for allowlists, enable DM pairing. When a teammate DMs the bot, they get a one-time pairing code. You approve it with `caesar pairing approve telegram XKGH5N7P` — simple and secure.
...
:::tip
On messaging platforms, sessions auto-reset after idle time (default: 24 hours) or daily at 4 AM. Adjust per-platform in `~/.caesar/config.yaml` if you need longer sessions.
:::
...
TERMINAL_DOCKER_IMAGE=caesar-sandbox:latest
```
...
Caesar checks every command against a curated list of dangerous patterns before execution. This includes recursive deletes, SQL drops, piping curl to shell, and more. Don't disable this in production — it exists for good reasons.
