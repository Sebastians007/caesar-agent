---
sidebar_position: 7
title: "Use Voice Mode with Caesar"
description: "A practical guide to setting up and using Caesar voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Caesar
...
- you want Caesar sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Caesar.
...
## Step 1: make sure normal Caesar works first
...
Caesar supports both local and cloud speech stacks.
...
### If you use `caesar setup`

If you choose NeuTTS in the setup wizard, Caesar checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Caesar transcribes and responds
...
- asking Caesar to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Caesar loop.

## Tuning CLI behavior

### Silence threshold

If Caesar starts/stops too aggressively, tune:
...
Caesar stays a normal chat bot, but can speak replies.
...
- you want Caesar to function like a portable research or ops assistant
...
Caesar joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Caesar detects speech boundaries
- transcripts are posted in the associated text channel
- Caesar responds in text and audio
...
1. get text Caesar working
2. install `caesar-agent[voice]`
