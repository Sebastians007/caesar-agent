---
sidebar_position: 5
title: "Use MCP with Caesar"
description: "A practical guide to connecting MCP servers to Caesar Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Caesar

This guide shows how to actually use MCP with Caesar Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Caesar tool
- you want Caesar to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Caesar to internal APIs, databases, or company systems without modifying Caesar core

Do not use MCP when:
- a built-in Caesar tool already solves the job well
...
- Caesar remains the agent
- MCP servers contribute tools
- Caesar discovers those tools at startup or reload time
...
If you installed Caesar with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Caesar banner/status should show MCP integration when configured
- ask Caesar what tools it has available
...
There are two categories of MCP-exposed functionality in Caesar:
...
2. Caesar-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Caesar will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Caesar to reason over a bounded workspace.
...
Now Caesar can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Caesar core.
...
Because Caesar now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
