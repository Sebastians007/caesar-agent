...
title: "Build a Caesar Plugin"
description: "Step-by-step guide to building a complete Caesar plugin with tools, hooks, data files, and skills"
---

# Build a Caesar Plugin

This guide walks through building a complete Caesar plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.caesar/plugins/calculator
cd ~/.caesar/plugins/calculator
```
...
This tells Caesar: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Caesar may pass additional context in the future.
...
- If this function crashes, the plugin is disabled but Caesar continues fine
...
def _install_skill():
    """Copy our skill to ~/.caesar/skills/ on first load."""
    try:
        from caesar_cli.config import get_caesar_home
        dest = get_caesar_home() / "skills" / "my-plugin" / "SKILL.md"
    except Exception:
        dest = Path.home() / ".caesar" / "skills" / "my-plugin" / "SKILL.md"
...
```toml
# pyproject.toml
[project.entry-points."caesar_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install caesar-plugin-calculator
# Plugin auto-discovered on next caesar startup
```
...
```python
# Wrong — will break if Caesar passes extra context
def handler(args):
    ...
