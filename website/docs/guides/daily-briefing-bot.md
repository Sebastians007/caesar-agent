...
2. **Caesar spins up** a fresh agent session with your prompt
...
- **Caesar Agent installed** — see the [Installation guide](/docs/getting-started/installation)
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.caesar/cron/output/` and you can read them anytime.
:::
...
Caesar will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Caesar Agent
```
...
Just tell Caesar what you want:
...
Caesar will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Caesar to delegate each topic to a sub-agent:
...
Caesar will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
