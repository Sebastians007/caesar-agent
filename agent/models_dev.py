...
Data is cached in memory (1hr TTL) and on disk (~/.caesar/models_dev_cache.json)
...
# Provider ID mapping: Caesar provider names → models.dev provider IDs
...
def _get_cache_path() -> Path:
    ...
    env_val = os.environ.get("CAESAR_HOME", "")
    caesar_home = Path(env_val) if env_val else Path.home() / ".caesar"
    return caesar_home / "models_dev_cache.json"
