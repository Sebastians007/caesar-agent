"""Anthropic Messages API adapter for Caesar Agent.

Translates between Caesar's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Caesar historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Caesar)
    ...
    # 1. Caesar-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Caesar configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# ── Caesar-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.caesar/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_CAESAR_OAUTH_FILE = get_caesar_home() / ".anthropic_oauth.json"
...
def run_caesar_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Caesar-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Caesar with your Claude Pro/Max subscription.")
    print()
...
def run_caesar_oauth_login() -> Optional[str]:
    """Run Caesar-native OAuth PKCE flow for Claude Pro/Max subscription.

    Opens a browser to claude.ai for authorization, prompts for the code,
    exchanges it for tokens, and stores them in ~/.caesar/.anthropic_oauth.json.

    Returns the access token on success, None on failure.
    """
    result = run_caesar_oauth_login_pure()
    if not result:
        return None
    ...
    _save_caesar_oauth_credentials(access_token, refresh_token, expires_at_ms)
...
def _save_caesar_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.caesar/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
    ...
    try:
        _CAESAR_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
        _CAESAR_OAUTH_FILE.write_text(json.dumps(data, indent=2), encoding="utf-8")
        _CAESAR_OAUTH_FILE.chmod(0o600)
    except (OSError, IOError) as e:
        logger.debug("Failed to save Caesar OAuth credentials: %s", e)


def read_caesar_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Caesar-managed OAuth credentials from ~/.caesar/.anthropic_oauth.json."""
    if _CAESAR_OAUTH_FILE.exists():
        try:
            data = json.loads(_CAESAR_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read Caesar OAuth credentials: %s", e)
    return None


def refresh_caesar_oauth_token() -> Optional[str]:
    """Refresh the Caesar-managed OAuth token using the stored refresh token.

    Returns the new access token, or None if refresh fails.
    """
    creds = read_caesar_oauth_credentials()
    ...
    try:
        ...
        _save_caesar_oauth_credentials(
            refreshed["access_token"],
            refreshed["refresh_token"],
            refreshed["expires_at_ms"],
        )
        ...
        logger.debug("Successfully refreshed Caesar OAuth token")
        return refreshed["access_token"]
    except Exception as e:
        logger.debug("Failed to refresh Caesar OAuth token: %s", e)
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Caesar Agent", "Claude Code")
                text = text.replace("Caesar agent", "Claude Code")
                text = text.replace("caesar-agent", "claude-code")
