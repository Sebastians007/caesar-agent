...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.caesar/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=caesar-agent"]}
...
_AUTH_JSON_PATH = get_caesar_home() / "auth.json"
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.caesar/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Caesar auth store."""
    ...
    try:
        from caesar_cli.auth import _read_codex_tokens
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "nous":
        ...
        if client is None:
            logger.warning("auxiliary.provider=nous but Nous Portal not configured (run: caesar login)")
        return client, model

    if forced == "codex":
        client, model = _try_codex()
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (run: caesar model)")
        return client, model
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: caesar login)")
            return None, None
        final_model = model or default
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: caesar model)")
                return None, None
    ...
    try:
        ...
        logger.debug("caesar_cli.auth not available for provider %s", provider)
        return None, None
...
def get_available_vision_backends() -> List[str]:
    ...
    it is also a known-good vision backend; otherwise Caesar falls back through
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=caesar-agent"])
    if merged_extra:
