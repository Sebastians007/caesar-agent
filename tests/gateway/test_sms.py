...
class TestSmsToolset:
    def test_caesar_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("caesar-sms")
        assert ts is not None
        assert "tools" in ts

    def test_caesar_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("caesar-gateway")
        assert gw is not None
        assert "caesar-sms" in gw["includes"]
