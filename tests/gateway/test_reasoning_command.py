...
class TestReasoningCommand:
    ...
    async def test_reasoning_command_reloads_current_state_from_config(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_run, "_caesar_home", caesar_home)
        monkeypatch.delenv("CAESAR_REASONING_EFFORT", raising=False)
    ...
    def test_run_agent_reloads_reasoning_config_per_message(self, tmp_path, monkeypatch):
        caesar_home = tmp_path / "caesar"
        caesar_home.mkdir()
        (caesar_home / "config.yaml").write_text("agent:\n  reasoning_effort: low\n", encoding="utf-8")

        monkeypatch.setattr(gateway_run, "_caesar_home", caesar_home)
        monkeypatch.setattr(gateway_run, "_env_path", caesar_home / ".env")
    ...
    def test_run_agent_prefers_config_over_stale_reasoning_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("CAESAR_REASONING_EFFORT", "low")
        fake_run_agent = types.ModuleType("run_agent")
        fake_run_agent.AIAgent = _CapturingAgent
        monkeypatch.setitem(sys.modules, "run_agent", fake_run_agent)
    ...
    def test_run_agent_homeassistant_uses_default_platform_toolset(self, tmp_path, monkeypatch):
        caesar_home = tmp_path / "caesar"
        caesar_home.mkdir()
        (caesar_home / "config.yaml").write_text("", encoding="utf-8")
