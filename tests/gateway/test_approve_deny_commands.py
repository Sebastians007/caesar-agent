...
class TestBlockingApprovalE2E:
    ...
    def test_blocking_approval_approve_once(self):
        ...
        def agent_thread():
            ...
            os.environ["CAESAR_SESSION_KEY"] = session_key
            try:
                result_holder[0] = check_all_command_guards(
                    "rm -rf /important", "local"
                )
            finally:
                os.environ.pop("CAESAR_EXEC_ASK", None)
                os.environ.pop("CAESAR_SESSION_KEY", None)
...
class TestFallbackNoCallback:
    ...
    def test_no_callback_returns_approval_required(self):
        ...
        os.environ["CAESAR_EXEC_ASK"] = "1"
        os.environ["CAESAR_SESSION_KEY"] = "no-callback-test"
        try:
