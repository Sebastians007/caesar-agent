"""
Tests for mcp_serve — Caesar MCP server.
...
def _isolate_caesar_home(tmp_path, monkeypatch):
    """Redirect CAESAR_HOME to a temp directory."""
    monkeypatch.setenv("CAESAR_HOME", str(tmp_path))
    try:
        import caesar_constants
        monkeypatch.setattr(caesar_constants, "get_caesar_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking caesar_state schema."""
