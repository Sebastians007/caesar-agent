"""Tests for caesar_cli configuration management."""
...
class TestGetCaesarHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("CAESAR_HOME", None)
            home = get_caesar_home()
            assert home == Path.home() / ".caesar"

    def test_env_override(self):
        with patch.dict(os.environ, {"CAESAR_HOME": "/custom/path"}):
            home = get_caesar_home()
            assert home == Path("/custom/path")


class TestEnsureCaesarHome:
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"CAESAR_HOME": str(tmp_path)}, clear=False):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from caesar_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
