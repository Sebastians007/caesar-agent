"""Comprehensive tests for caesar_cli.profiles module.
...
from caesar_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_caesar_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and CAESAR_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.caesar/profiles)
    * CAESAR_HOME  -> tmp_path/.caesar  (so get_caesar_home() agrees)
    * Creates the bare-minimum ~/.caesar directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".caesar"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("CAESAR_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_caesar_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".caesar"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".caesar" / "profiles" / "coder"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("caesar_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".caesar" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_caesar_home_returns_default(self, profile_env):
        # CAESAR_HOME points to tmp_path/.caesar which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".caesar" / "profiles" / "coder"
        monkeypatch.setenv("CAESAR_HOME", str(profile_dir))
    ...
    def test_custom_path_returns_custom(self, profile_env, monkeypatch):
        ...
        monkeypatch.setenv("CAESAR_HOME", str(custom))
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("caesar")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".caesar" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("caesar_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".caesar" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("caesar-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".caesar_history",
        ...
        excluded_prefixes = [
            "default/caesar-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.caesar_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_caesar_profiles_function(self):
        script = generate_bash_completion()
        assert "_caesar_profiles" in script

    def test_zsh_completion_has_caesar_function(self):
        script = generate_zsh_completion()
        assert "_caesar" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultCaesarHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_caesar_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".caesar" / "profiles"

    def test_default_caesar_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_caesar_home()
        assert home == tmp_path / ".caesar"
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".caesar" / "profiles" / "mybot"
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from caesar_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".caesar"
