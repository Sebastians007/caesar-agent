"""Tests for the update check mechanism in caesar_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from caesar_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path):
    ...
    repo_dir = tmp_path / "caesar-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("caesar_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("caesar_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path):
    ...
    with patch("caesar_cli.banner.os.getenv", return_value=str(tmp_path)):
        with patch("caesar_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
            result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path):
    ...
    import caesar_cli.banner as banner

    # Create a fake banner.py so the fallback path also has no .git
    fake_banner = tmp_path / "caesar_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root():
    """Dev install: falls back to Path(__file__).parent.parent when CAESAR_HOME has no git repo."""
    import caesar_cli.banner as banner
    ...
    # Point CAESAR_HOME at a temp dir with no caesar-agent/.git
    import tempfile
    with tempfile.TemporaryDirectory() as td:
        with patch("caesar_cli.banner.os.getenv", return_value=td):
            with patch("caesar_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from caesar_cli.main import _invalidate_update_cache

    # Build a fake ~/.caesar with default + two named profiles
