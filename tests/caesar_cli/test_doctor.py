...
import caesar_cli.doctor as doctor
import caesar_cli.gateway as gateway_cli
from caesar_cli import doctor as doctor_mod
from caesar_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "CAESAR_HOME", caesar_home)
    monkeypatch.delenv("CAESAR_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("CAESAR_INTERACTIVE")
        raise SystemExit(0)
