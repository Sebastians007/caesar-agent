...
import caesar_cli.gateway as gateway_cli


class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "caesar-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.caesar.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "start", "ai.caesar.gateway"])
            ),
        )
...
class TestSystemUnitCaesarHome:
    """CAESAR_HOME in system units must reference the target user, not root."""

    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        # Simulate sudo: Path.home() returns /root, target user is alice
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("CAESAR_HOME", raising=False)
        monkeypatch.setattr(
        ...
        assert 'CAESAR_HOME=/home/alice/.caesar' in unit
        assert '/root/.caesar' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: CAESAR_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("CAESAR_HOME", "/root/.caesar/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'CAESAR_HOME=/home/alice/.caesar/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_caesar_home(self, monkeypatch):
        # Custom CAESAR_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("CAESAR_HOME", "/opt/caesar-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'CAESAR_HOME=/opt/caesar-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's CAESAR_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        caesar_home = str(gateway_cli.get_caesar_home().resolve())
        assert f'CAESAR_HOME={caesar_home}' in unit


class TestCaesarHomeForTargetUser:
    """Unit tests for _caesar_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("CAESAR_HOME", raising=False)

        result = gateway_cli._caesar_home_for_target_user("/home/alice")
        assert result == "/home/alice/.caesar"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._caesar_home_for_target_user("/home/alice")
        assert result == "/home/alice/.caesar/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("CAESAR_HOME", "/opt/caesar")

        result = gateway_cli._caesar_home_for_target_user("/home/alice")
        assert result == "/opt/caesar"
