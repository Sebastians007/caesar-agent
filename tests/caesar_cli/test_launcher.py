"""Tests for the top-level `./caesar` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./caesar` should use `caesar_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "caesar"
    called = []

    fake_main_module = types.ModuleType("caesar_cli.main")

    def fake_main():
        called.append("caesar_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "caesar_cli.main", fake_main_module)
