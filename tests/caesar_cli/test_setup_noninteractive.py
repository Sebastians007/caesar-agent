...
class TestNonInteractiveSetup:
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("caesar_cli.setup.ensure_caesar_home"),
            patch("caesar_cli.setup.load_config", return_value={}),
            patch("caesar_cli.setup.get_caesar_home", return_value="/tmp/.caesar"),
            patch("caesar_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "caesar config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `caesar` should not prompt for input when no provider exists and stdin is headless."""
        from caesar_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("caesar_cli.main._has_any_provider_configured", return_value=False),
            patch("caesar_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_returning_user_terminal_menu_choice_dispatches_terminal_section(self, tmp_path):
        ...
        from caesar_cli import setup as setup_mod
        ...
        with (
            patch.object(setup_mod, "ensure_caesar_home"),
            patch.object(setup_mod, "load_config", return_value=config),
            patch.object(setup_mod, "get_caesar_home", return_value=tmp_path),
            ...
            patch("caesar_cli.auth.get_active_provider", return_value=None),
