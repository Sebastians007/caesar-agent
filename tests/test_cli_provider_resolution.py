...
from caesar_cli.auth import AuthError
from caesar_cli import main as caesar_main
...
def test_caesar_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in CaesarCLI.__init__")

    monkeypatch.setattr("caesar_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("caesar_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.CaesarCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("caesar_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("caesar_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.CaesarCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("CAESAR_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.CaesarCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("caesar_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "caesar_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.CaesarCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setenv("CAESAR_ENABLE_NOUS_MANAGED_TOOLS", "1")
    config = {
    ...
    monkeypatch.setattr(
        "caesar_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "caesar_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "caesar_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("caesar_cli.auth._prompt_model_selection", lambda model_ids, current_model="": "claude-opus-4-6")
    monkeypatch.setattr("caesar_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("caesar_cli.auth._update_config_for_provider", lambda provider, url: None)
    monkeypatch.setattr(
        "caesar_cli.nous_subscription.get_nous_subscription_explainer_lines",
        lambda: ["Nous subscription enables managed web tools."],
    )

    caesar_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.CaesarCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.CaesarCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    ...
    monkeypatch.setattr("caesar_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("caesar_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("caesar_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("caesar_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(caesar_main, "_prompt_provider_choice", lambda choices: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    caesar_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "caesar_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("caesar_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("caesar_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("caesar_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("caesar_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "caesar_cli.models.probe_api_models",
    ...
    caesar_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(caesar_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "caesar_cli.config.load_config",
    ...
    monkeypatch.setattr("caesar_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("caesar_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("caesar_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("caesar_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(caesar_main, "_prompt_provider_choice", lambda choices: 0)
    ...
    monkeypatch.setattr("caesar_cli.auth._login_nous", _fake_login)

    caesar_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="caesar-local",
