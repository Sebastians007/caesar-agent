...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Caesar Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_caesar_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("CAESAR_HOME", str(tmp_path / ".caesar"))

    caesar_env = tmp_path / ".caesar" / ".env"
    caesar_env.parent.mkdir(parents=True)
    caesar_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.caesar/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
