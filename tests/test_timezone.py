"""
Tests for timezone support (caesar_time module + integration points).
...
# =========================================================================
# caesar_time.now() — core helper
# =========================================================================

class TestCaesarTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        caesar_time.reset_cache()

    def teardown_method(self):
        caesar_time.reset_cache()
        os.environ.pop("CAESAR_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["CAESAR_TIMEZONE"] = "Asia/Kolkata"
        result = caesar_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["CAESAR_TIMEZONE"] = "UTC"
        result = caesar_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["CAESAR_TIMEZONE"] = "America/New_York"
        result = caesar_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["CAESAR_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="caesar_time"):
            result = caesar_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["CAESAR_TIMEZONE"] = "Europe/London"
        tz = caesar_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["CAESAR_TIMEZONE"] = "Not/A/Timezone"
        tz = caesar_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["CAESAR_TIMEZONE"] = "Asia/Tokyo"
        assert caesar_time.get_timezone_name() == "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When CAESAR_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["CAESAR_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When CAESAR_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("CAESAR_TIMEZONE", None)
    ...
    def test_caesar_timezone_not_leaked_to_child(self):
        """CAESAR_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["CAESAR_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("CAESAR_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=caesar_tz) which shifted
        absolute time when system-local tz != Caesar tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_caesar_tz(self):
        """Already-aware datetimes should be normalized to Caesar tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Caesar tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Caesar tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Caesar is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Caesar tz
        ...
        # Use a Caesar timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _caesar_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["CAESAR_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        caesar_time.reset_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Caesar timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["CAESAR_TIMEZONE"] = "US/Eastern"
        caesar_time.reset_cache()
