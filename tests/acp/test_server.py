"""Tests for acp_adapter.server — CaesarACPAgent ACP server."""
...
from acp_adapter.server import CaesarACPAgent, CAESAR_VERSION
...
def agent(mock_manager):
    """CaesarACPAgent backed by a mock session manager."""
    return CaesarACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "caesar-agent"
        assert resp.agent_info.version == CAESAR_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert CAESAR_VERSION in result
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("caesar_cli.config.load_config", lambda: {
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = CaesarACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["caesar-acp"]
