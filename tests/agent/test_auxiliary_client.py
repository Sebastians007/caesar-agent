...
class TestReadCodexAccessToken:
    def test_valid_auth_store(self, tmp_path, monkeypatch):
        ...
        (caesar_home / "auth.json").write_text(json.dumps({
            "version": 1,
            "providers": {
                "openai-codex": {
...
class TestExpiredCodexFallback:
    ...
    def test_caesar_oauth_file_sets_oauth_flag(self, monkeypatch):
        """OAuth-style tokens should get is_oauth=*** (token is not sk-ant-api-*)."""
        # Mock resolve_anthropic_token to return an OAuth-style token
        with patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="caesar-oauth-jwt-token"), \
             patch("agent.anthropic_adapter.build_anthropic_client") as mock_build, \
             patch("agent.auxiliary_client._select_pool_entry", return_value=(False, None)):
            mock_build.return_value = MagicMock()
            from agent.auxiliary_client import _try_anthropic, AnthropicAuxiliaryClient
...
class TestGetTextAuxiliaryClient:
    ...
    def test_custom_endpoint_over_codex(self, monkeypatch, codex_auth_dir):
        ...
        monkeypatch.setattr("caesar_cli.config.load_config", lambda: config)
        monkeypatch.setattr("caesar_cli.runtime_provider.load_config", lambda: config)
    ...
    def test_codex_pool_entry_takes_priority_over_auth_store(self):
        ...
        with (
            ...
            patch("caesar_cli.auth._read_codex_tokens", side_effect=AssertionError("legacy codex store should not run")),
        ):
...
class TestAuxiliaryPoolAwareness:
    ...
    def test_selected_anthropic_provider_is_preferred_for_vision_auto(self, monkeypatch):
        ...
        with (
            ...
            patch("agent.anthropic_adapter.resolve_anthropic_token", return_value="sk-ant-api03-key"),
            patch("agent.auxiliary_client.OpenAI") as mock_openai,
            patch("caesar_cli.config.load_config", fake_load_config),
        ):
            client, model = get_vision_auxiliary_client()
