...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "CAESAR_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["CAESAR_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_nous_auth_token_respects_caesar_home_override(self, tmp_path):
        """Auth lookup should read from CAESAR_HOME/auth.json, not ~/.caesar/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".caesar").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``caesar tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "CAESAR_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
