...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("caesar_cli.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("caesar_cli.config.load_config", return_value={"mcp_servers": servers}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("caesar_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_caesar_schema(self):
...
class TestToolsetInjection:
    def test_mcp_tools_added_to_all_caesar_toolsets(self):
        """Discovered MCP tools are dynamically injected into all caesar-* toolsets."""
        from tools.mcp_tool import MCPServerTask
        ...
        fake_toolsets = {
            "caesar-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
            "caesar-telegram": {"tools": ["terminal"], "description": "TG", "includes": []},
            "caesar-gateway": {"tools": [], "description": "GW", "includes": []},
            "non-caesar": {"tools": [], "description": "other", "includes": []},
        }
        ...
        # All caesar-* toolsets get injection
        assert "mcp_fs_list_files" in fake_toolsets["caesar-cli"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["caesar-telegram"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["caesar-gateway"]["tools"]
        # Non-caesar toolset should NOT get injection
        assert "mcp_fs_list_files" not in fake_toolsets["non-caesar"]["tools"]
        # Original tools preserved
        assert "terminal" in fake_toolsets["caesar-cli"]["tools"]
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "caesar-cli": {"tools": [], "description": "CLI", "includes": []},
        }
