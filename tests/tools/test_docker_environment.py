...
def test_execute_uses_caesar_dotenv_for_allowlisted_env(monkeypatch):
    ...
    monkeypatch.setattr(docker_env, "_load_caesar_env_vars", lambda: {"GITHUB_TOKEN": "value_from_dotenv"})
...
def test_execute_prefers_shell_env_over_caesar_dotenv(monkeypatch):
    env = _make_execute_only_env(["GITHUB_TOKEN"])
    popen_calls = []
