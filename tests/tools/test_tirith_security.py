...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._caesar_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_caesar_bin(self):
        """After _INSTALL_FAILED, manual install in CAESAR_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        caesar_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(caesar_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(caesar_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._caesar_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == caesar_bin
            assert _tirith_mod._resolved_path == caesar_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# CAESAR_HOME isolation
# ---------------------------------------------------------------------------

class TestCaesarHomeIsolation:
    def test_caesar_bin_dir_respects_caesar_home(self):
        """_caesar_bin_dir must use CAESAR_HOME, not hardcoded ~/.caesar."""
        from tools.tirith_security import _caesar_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"CAESAR_HOME": tmpdir}):
            result = _caesar_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_caesar_home(self):
        """_failure_marker_path must use CAESAR_HOME, not hardcoded ~/.caesar."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"CAESAR_HOME": "/custom/caesar"}):
            result = _failure_marker_path()
        assert result == "/custom/caesar/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets CAESAR_HOME; verify it's active."""
        caesar_home = os.getenv("CAESAR_HOME")
        assert caesar_home is not None, "CAESAR_HOME should be set by conftest"
        assert "caesar_test" in caesar_home, "Should point to test temp dir"

    def test_get_caesar_home_fallback(self):
        """Without CAESAR_HOME set, falls back to ~/.caesar."""
        from tools.tirith_security import _get_caesar_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove CAESAR_HOME entirely
            os.environ.pop("CAESAR_HOME", None)
            result = _get_caesar_home()
        assert result == os.path.join(os.path.expanduser("~"), ".caesar")
