...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("caesar_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("caesar_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestTeePattern:
    ...
    def test_tee_caesar_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.caesar/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_caesar_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $CAESAR_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_caesar_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$CAESAR_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_caesar_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $CAESAR_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.caesar/caesar-agent && source venv/bin/activate && python -m caesar_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m caesar_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m caesar_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "caesar_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_not_flagged(self):
        ...
        cmd = "systemctl --user restart caesar-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_caesar_detected(self):
        """pkill targeting caesar/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_caesar_detected(self):
        cmd = "killall caesar"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
