...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        caesar_home.mkdir()
        (caesar_home / "token.json").write_text("{}")
        ...
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(caesar_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.caesar/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        (caesar_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        (caesar_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        (caesar_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mount["container_path"] == "/root/.caesar/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        caesar_home = tmp_path / ".caesar"
        (caesar_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"CAESAR_HOME": str(caesar_home)}):
            mount = get_skills_directory_mount(container_base="/home/user/.caesar")

        assert mount["container_path"] == "/home/user/.caesar/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.caesar/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.caesar/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape CAESAR_HOME."""
        monkeypatch.setenv("CAESAR_HOME", str(tmp_path / ".caesar"))
        (tmp_path / ".caesar").mkdir()

        # Create a sensitive file one level above caesar_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside caesar_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside CAESAR_HOME must still be registered."""
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        monkeypatch.setenv("CAESAR_HOME", str(caesar_home))
        (caesar_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_caesar_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of CAESAR_HOME must be allowed."""
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        subdir = caesar_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside CAESAR_HOME pointing outside must be rejected."""
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        monkeypatch.setenv("CAESAR_HOME", str(caesar_home))

        # Create a sensitive file outside caesar_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside caesar_home pointing outside
        symlink = caesar_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes CAESAR_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, caesar_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape CAESAR_HOME."""
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        monkeypatch.setenv("CAESAR_HOME", str(caesar_home))
        ...
        sensitive.write_text("{}")
        self._write_config(caesar_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(caesar_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside CAESAR_HOME via config must still mount."""
        caesar_home = tmp_path / ".caesar"
        caesar_home.mkdir()
        monkeypatch.setenv("CAESAR_HOME", str(caesar_home))

        (caesar_home / "oauth.json").write_text("{}")
        self._write_config(caesar_home, ["oauth.json"])
