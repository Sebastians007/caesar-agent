"""Tests for --yolo (CAESAR_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When CAESAR_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("CAESAR_INTERACTIVE", "1")
        monkeypatch.setenv("CAESAR_SESSION_KEY", "test-session")
        monkeypatch.delenv("CAESAR_YOLO_MODE", raising=False)
        monkeypatch.delenv("CAESAR_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("CAESAR_EXEC_ASK", raising=False)
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With CAESAR_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("CAESAR_YOLO_MODE", "1")
        monkeypatch.setenv("CAESAR_INTERACTIVE", "1")
        monkeypatch.setenv("CAESAR_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """CAESAR_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("CAESAR_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for CAESAR_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("CAESAR_YOLO_MODE", "")
        monkeypatch.setenv("CAESAR_INTERACTIVE", "1")
        monkeypatch.setenv("CAESAR_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("CAESAR_YOLO_MODE") returns ""
