...
def _restore_tool_modules():
    original_caesar_home = os.environ.get("CAESAR_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_caesar_home is None:
            os.environ.pop("CAESAR_HOME", None)
        else:
            os.environ["CAESAR_HOME"] = original_caesar_home
        _reset_modules(("tools", "caesar_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    caesar_cli = types.ModuleType("caesar_cli")
    caesar_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["caesar_cli"] = caesar_cli
    caesar_home = tmp_path / "caesar-home"
    os.environ["CAESAR_HOME"] = str(caesar_home)
    sys.modules["caesar_cli.config"] = types.SimpleNamespace(
        get_caesar_home=lambda: caesar_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="caesar-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": caesar_home / "modal_snapshots.json",
