...
class TestRegisterServerTools:
    ...
    def test_injects_caesar_toolsets(self, mock_registry, mock_toolsets):
        """Tools are injected into caesar-* toolsets but not custom ones."""
        ...
        # Injected into caesar-* toolsets
        assert "mcp_my_srv_my_tool" in mock_toolsets["caesar-cli"]["tools"]
        assert "mcp_my_srv_my_tool" in mock_toolsets["caesar-telegram"]["tools"]
        # NOT into non-caesar toolsets
...
class TestRefreshTools:
    ...
    async def test_nuke_and_repave(self, mock_registry, mock_toolsets):
        ...
        mock_toolsets["caesar-cli"]["tools"].append("mcp_live_srv_old_tool")
        ...
        assert "mcp_live_srv_old_tool" not in mock_toolsets["caesar-cli"]["tools"]

        # New tool registered
        assert "mcp_live_srv_new_tool" in mock_registry.get_all_tool_names()
        assert "mcp_live_srv_new_tool" in mock_toolsets["caesar-cli"]["tools"]
