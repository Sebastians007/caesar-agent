...
def _isolate(tmp_path, monkeypatch):
    """Isolate CAESAR_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal CaesarCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = CaesarCLI.__new__(CaesarCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["caesar-core"]
        obj.compact = False
