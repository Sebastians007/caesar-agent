...
class TestConfigFilePermissions(unittest.TestCase):
    ...
    def test_save_config_sets_0600(self):
        ...
        with patch("caesar_cli.config.get_config_path", return_value=config_path), \
             patch("caesar_cli.config.ensure_caesar_home"):
            from caesar_cli.config import save_config
    ...
    def test_save_env_value_sets_0600(self):
        ...
        with patch("caesar_cli.config.get_env_path", return_value=env_path), \
             patch("caesar_cli.config.ensure_caesar_home"):
            from caesar_cli.config import save_env_value
            save_env_value("TEST_KEY", "test_value")

            file_mode = stat.S_IMODE(os.stat(env_path).st_mode)
            self.assertEqual(file_mode, 0o600)

    def test_ensure_caesar_home_sets_0700(self):
        home = Path(self.tmpdir) / ".caesar"
        with patch("caesar_cli.config.get_caesar_home", return_value=home):
