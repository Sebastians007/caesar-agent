"""Tests for caesar_cli.plugins_cmd — the ``caesar plugins`` CLI subcommand."""
...
from caesar_cli.plugins_cmd import (
    _copy_example_files,
    _read_manifest,
    _repo_name_from_url,
    _resolve_git_url,
...
class TestPluginsCommandDispatch:
    ...
    @patch("caesar_cli.plugins_cmd.cmd_remove")
    ...
    @patch("caesar_cli.plugins_cmd.cmd_update")
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="caesar_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from caesar_cli.plugins_cmd import cmd_install
        import argparse

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("caesar_cli.plugins_cmd._resolve_git_url")
...
class TestCmdUpdate:
    ...
    @patch("caesar_cli.plugins_cmd._sanitize_plugin_name")
    @patch("caesar_cli.plugins_cmd._plugins_dir")
    @patch("caesar_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from caesar_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    @patch("caesar_cli.plugins_cmd._sanitize_plugin_name")
    @patch("caesar_cli.plugins_cmd._plugins_dir")
    @patch("caesar_cli.plugins_cmd.shutil.rmtree")
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from caesar_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from caesar_cli.plugins_cmd import cmd_list
    ...
    @patch("caesar_cli.plugins_cmd._plugins_dir")
    @patch("caesar_cli.plugins_cmd._read_manifest")
...
class TestCopyExampleFiles:
    ...
    def test_copies_example_files(self, tmp_path):
        from caesar_cli.plugins_cmd import _copy_example_files
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "caesar_cli.plugins_cmd.shutil.copy2",
