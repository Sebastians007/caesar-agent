...
def test_round_robin_strategy_rotates_priorities(tmp_path, monkeypatch):
    ...
    config_path = tmp_path / "caesar" / "config.yaml"
...
def test_mark_exhausted_and_rotate_persists_status(tmp_path, monkeypatch):
    ...
    auth_payload = json.loads((tmp_path / "caesar" / "auth.json").read_text())
...
def test_try_refresh_current_updates_only_current_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "caesar_cli.auth.refresh_codex_oauth_pure",
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.read_caesar_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
...
def test_singleton_seed_does_not_clobber_manual_oauth_entry(tmp_path, monkeypatch):
    ...
    assert {entry.source for entry in entries} == {"manual:caesar_pkce", "caesar_pkce"}
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("CAESAR_HOME", str(tmp_path / "caesar"))
    (tmp_path / "caesar").mkdir(parents=True, exist_ok=True)
    import yaml
    config_path = tmp_path / "caesar" / "config.yaml"
    config_path.write_text(yaml.dump({
        "custom_providers": [
