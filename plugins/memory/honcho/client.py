...
Resolution order for config file:
  1. $CAESAR_HOME/honcho.json  (instance-local, enables isolated Caesar instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Caesar profile.

    Resolution order:
      1. CAESAR_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``caesar.<profile>``
      3. Fallback: ``"caesar"`` (default profile)
    """
    explicit = os.environ.get("CAESAR_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $CAESAR_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.caesar/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_caesar_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = Path.home() / ".caesar" / "honcho.json"
...
def _resolve_memory_mode(
    ...
    Object form:  { "default": "hybrid", "caesar": "honcho", ... }
...
class HonchoClientConfig:
    ...
    # Config object form: "memoryMode": { "default": "hybrid", "caesar": "honcho" }
    ...
    # Max chars of dialectic result to inject into Caesar system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.caesar
    ...
    def from_global_config(
        ...
        Resolution: $CAESAR_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Caesar profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.caesar block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Caesar session title (from /title command)
          3. per-session strategy — Caesar session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Caesar session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'caesar honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            caesar_cfg = load_config()
            honcho_cfg = caesar_cfg.get("honcho", {})
