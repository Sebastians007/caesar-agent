...
Handles: caesar honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "caesar" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Caesar profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from caesar_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'caesar honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'caesar honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `caesar update` -- no output, no exceptions.
    """
    try:
        from caesar_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Caesar profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $CAESAR_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_caesar_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Caesar persistent cross-session memory.")
    ...
    caesar_host = hosts.setdefault(_host_key(), {})
    ...
    if not effective_key:
        ...
        print("  Run 'caesar honcho setup' again once you have a key.\n")
        return

    # Peer name
    current_peer = caesar_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        caesar_host["peerName"] = new_peer

    current_workspace = caesar_host.get("workspace") or cfg.get("workspace", "caesar")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        caesar_host["workspace"] = new_workspace

    caesar_host.setdefault("aiPeer", _host_key())

    # Memory mode
    current_mode = caesar_host.get("memoryMode") or cfg.get("memoryMode", "hybrid")
    print("\n  Memory mode options:")
    ...
    if new_mode in ("hybrid", "honcho"):
        caesar_host["memoryMode"] = new_mode
    else:
        caesar_host["memoryMode"] = "hybrid"

    # Write frequency
    current_wf = str(caesar_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    ...
    try:
        caesar_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        caesar_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # Recall mode
    _raw_recall = caesar_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        caesar_host["recallMode"] = new_recall

    # Session strategy
    current_strat = caesar_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy options:")
    print("    per-directory — one session per working directory (default)")
    print("    per-session   — new Honcho session each run, named by Caesar session ID")
    print("    per-repo      — one session per git repository (uses repo root name)")
    print("    global        — single session across all directories")
    new_strat = _prompt("Session strategy", default=current_strat)
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        caesar_host["sessionStrategy"] = new_strat

    caesar_host.setdefault("enabled", True)
    caesar_host.setdefault("saveMessages", True)
    ...
    print("\n  Other commands:")
    print("    caesar honcho status     — show full config")
    print("    caesar honcho mode       — show or change memory mode")
    print("    caesar honcho tokens     — show or set token budgets")
    print("    caesar honcho identity   — seed or show AI peer identity")
    print("    caesar honcho map <name> — map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Caesar profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: caesar honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'caesar honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: caesar honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        caesar = hosts.get(_host_key(), {})
        user = caesar.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = caesar.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = caesar.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = caesar.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Caesar' identity in Honcho. Seed with 'caesar honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print("\n  Set with: caesar honcho mode [hybrid|honcho]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = caesar.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = caesar.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = caesar.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Caesar asks Honcho's AI peer a question")
        ...
        print("\n  Set with: caesar honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'caesar honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'caesar honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    caesar honcho identity --show        — show both peer representations")
        print("    caesar honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Caesar + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Caesar\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Caesar persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  caesar honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'caesar honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'caesar honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  caesar honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'caesar'")
        print("  with Honcho configured and no prior session history.")
        print("  (Caesar calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    caesar honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'caesar honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Caesar, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'caesar honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    caesar honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  caesar honcho identity <path/to/SOUL.md>")
    ...
    print("    Caesar:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Caesar:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Caesar:   Honcho observes every message and updates representations")
    ...
    print("    Caesar:   per-session by default — each run gets its own session")
    print("              Map a custom name:  caesar honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. caesar honcho setup              — configure API key (required)")
        print("  2. caesar honcho migrate            — re-run this walkthrough")
    else:
        print("  1. caesar honcho status             — verify Honcho connection")
        print("  2. caesar                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. caesar honcho identity --show    — verify AI peer representation")
        print("  4. caesar honcho tokens             — tune context and dialectic budgets")
        print("  5. caesar honcho mode               — view or change memory mode")
    print()
